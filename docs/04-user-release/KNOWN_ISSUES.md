...
- Knowledge Graph is usable as a visual canvas workbench with selected-node edge states, a laconic inspector/overlay layer, split CSS ownership and first inspector/labels/canvas-controls/canvas-renderer JS ownership split, but richer day-to-day relationship operations, remaining action-handler and context-menu JavaScript split, lifecycle hardening and concept rethink remain backlog work.
