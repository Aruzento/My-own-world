...
6. Открыть презентацию.
7. Проверить backup/restore.
8. Создать task tracker и задачу.
9. Создать сущность `Правила`, импортировать legacy rule-карточку и проверить, что правило сохраняется после перезагрузки.
10. В карточке персонажа выбрать правило из Rule Tree в блоке `Эффекты и состояния` и проверить, что оно сохраняется.
11. В Rule Tree добавить условие к правилу, экспортировать JSON-пакет, импортировать пакет обратно и проверить предпросмотр активных эффектов.
12. Проверить Rule Tree engine: правило с `level >= 3` не должно применяться к персонажу 1 уровня и должно примениться после повышения уровня.
13. В Rule Tree сохранить package-файл в workspace, обновить список пакетов, импортировать файл без конфликта и проверить, что конфликтующий rule id показывает предупреждение вместо перезаписи.
