...
5. На карте проверить компактный title chip, тонкий icon-only toolbar, popup-ы, правый property Inspector и кастомное ПКМ-меню объекта: кнопки toolbar должны быть маленькими, без видимого текста, с tooltip при наведении; `Рука` должна быть среди инструментов; duplicate scene/layer panels не должны появляться поверх stage. Через toolbar открыть карту/сетку/слои/рисование/туман, а при выделении токена или фигуры Inspector должен давать редактируемые поля position/size/rotation/visibility/style; при multi-select он должен показывать сколько объектов видно/скрыто и давать групповые `Скрыть` / `Показать`.
6. Открыть `Граф связей` и проверить visible-slice clarity: counters `показано` / `в мире` / `скрыто`, статус `показано X из Y`, понятную причину скрытого среза и действия `Все связи` / `Уточнить поиск`. Кликнуть node и проверить, что связанные линии подсвечены, нерелевантный видимый контекст приглушен, а inspector показывает incoming/outgoing связи и действия `Открыть` / `Соседи`.
7. Открыть презентацию.
8. Проверить backup/restore.
9. Создать task tracker и задачу.
10. Создать сущность `Правила`, импортировать legacy rule-карточку и проверить, что правило сохраняется после перезагрузки.
11. В карточке персонажа выбрать правило из Rule Tree в блоке `Эффекты и состояния` и проверить, что оно сохраняется.
12. В Rule Tree добавить условие к правилу, экспортировать JSON-пакет, импортировать пакет обратно и проверить предпросмотр активных эффектов.
13. Проверить Rule Tree engine: правило с `level >= 3` не должно применяться к персонажу 1 уровня и должно примениться после повышения уровня.
14. В Rule Tree сохранить package-файл в workspace, обновить список пакетов, импортировать файл без конфликта и проверить, что конфликтующий rule id показывает предупреждение вместо перезаписи.
