...
6. Открыть `Граф связей` и проверить visible-slice clarity: counters `показано` / `в мире` / `скрыто`, статус `показано X из Y`, понятную причину скрытого среза и действия `Все связи` / `Уточнить поиск`.
