...
- Полная оценка находилась в `Тех. зрелость/01.06.2026 - оценка.md` в Git до personal-export retirement 2026-09-15; это историческая ссылка на прежнее дерево, не текущий документ.
...
- Бриф `MY_OWN_WORLD_AI_ARCHITECTURE_RISK_BRIEF_01_06_2026_UPDATED.docx` подтверждает зрелость **3.8 / 5** и уточняет порядок рисков: schema validation, backup/recovery, performance gate, visual regression, popup lifecycle.
...
6. **Разрез крупных файлов и CSS**: продолжать дробить `toolbar.js`, `editor.js`, `campaignMapPresentation.js`, `campaign-map.css`, `popup.css`.
