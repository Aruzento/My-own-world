---
summary: "Release notes and dated verification history before the current combat handoff"
read_when:
  - "When investigating the dated implementation evidence extracted from release-notes.md"
owner_zone: "archive"
---

# Release notes and dated verification history before the current combat handoff
...
- The component catalogue remains the design-system reference for shared Button/Input/Panel/Popover states, but `0.0.1.8.17` moved it behind a dev/test flag so normal Tools stays product-focused.
...
- UI redesign Phase 0 now has `UI_MIGRATION_BASELINES.md` and expanded visual smoke attachments for shell, Settings maintenance, tree, editor, Properties, map, graph, task tracker, Help/Support and shared popover surfaces before broader migration.
...
- Shared popup dragging now keeps the grabbed point under the pointer across ordinary fixed popups, centered-transform popups such as `Поиск и команды`, transform-based opening animations and right/inset-positioned popups. The fix lives in `popupManager` and does not add per-popup drag handlers.
...
- UI redesign Phase 5 core content is now usable: the root shell exposes `data-core-content-migration="0.0.1.8.11.7"`, tree/search have stable core-content zone markers, editor block drag-and-drop is restored with pointer preview/placeholder behavior, the first-level `Add block` popup now uses local sprite icons/grouped readable copy/focus states, the card editor header/runtime toolbar layer uses shared tokens with a visual guard against toolbar/title overlap, Properties fields use readable field variants/states with local sprite badges, ordinary card blocks now share one outer visual language with runtime-only type badges and thin colored markers, card-block dropdowns use the shared dark select styling, saved page templates are reachable from the create menu through `Из шаблона`, and `Поиск и команды` opens a deep PageRepository-backed command palette from the rail or `Ctrl+K`.
- UI redesign Phase 6 campaign map is now closed at `Usable` through the `0.0.1.8.12.10` post-review correction: the default map screen is canvas-first, with a compact title chip, a full-width top scene/session action bar, a full-height left icon-only canvas tool rail, body-level hover/focus tooltips that are not clipped inside buttons, no internal toolbar scrollbars, pan/Hand treated as a real tool, and no duplicate scene/layer panels over the stage. Add/picker, map image, grid, drawing, fog, shapes, layers, initiative and music still open through one compact map popup frame; selecting a token or shape opens a right-side editable Inspector for position, size, rotation, visibility and shape style; multi-selected tokens/shapes show visible/hidden counters and can be hidden from or shown to players as one group. Right-clicking a map object suppresses the browser menu and opens the custom compact action popup.
- UI redesign Phase 7 Knowledge Graph is now closed at `Usable` with `0.0.1.8.13.1`-`0.0.1.8.13.11`: the graph canvas keeps hidden-slice clarity but presents it more visually, with compact status chips, a slice meter, icon-first toolbar/filter/node actions, short selected-node relation chips, and relationship editing collapsed behind `Связи`. Graph CSS is split into base, slice, inspector and overlay/menu owner files. JS ownership now covers inspector helpers, node icons, labels/options, canvas controls, renderer, canvas actions, canvas overlays, relationship/context-menu HTML, view-state helpers and manual relationship persistence through `PageRecord`, `PageCommandService` and the write queue.
- UI redesign Phase 8 secondary screens is closed at `Usable` through `0.0.1.8.14.7`: task tracker now uses a compact workbench toolbar, Settings is one maintenance panel, Tools help opens a Help/Support/Release guide, and Tools includes a real `Пакеты мира` manager for branch/world export, package library, JSON import preview, backup-gated page/rulePackage/asset import, non-destructive conflict modes (`Стоп`, `Только новые`, `Копии`), embedded rulePackage apply, asset preflight and valid binary asset payload copy without overwriting existing files.
- UI redesign Phase 9 polish has started with `0.0.1.8.15.1`: Settings has a real `contrast` theme, appearance controls expose pressed/focus states, legacy editor/table/property/item/wiki focus gaps are covered, large map/graph inspectors no longer use expensive blur, and `npm run verify` now includes a UI polish audit.
- UI redesign Phase 9 performance now has `0.0.1.8.15.2`: a focused browser smoke renders large synthetic tree, campaign map, Knowledge Graph and task tracker workloads, then checks DOM counts and soft runtime budgets for the migrated workbench surfaces.
- UI redesign Phase 9 visual theme/scale coverage now has `0.0.1.8.15.3`: visual regression captures dark compact, contrast large and contrast narrow workbench baselines with an open tree, hidden right-panel foundation and populated card editor. The pass also removes hidden horizontal overflow from AppShell/topbar sizing and keeps Properties resize handles inside their fields.
- UI redesign Phase 9 dead CSS cleanup now has `0.0.1.8.15.4`: the retired campaign-map layer/object dock and scene-state inspector CSS/JS owner files are removed from the active bundle. The default map UI remains the split top scene/session bar, left tool rail, shared map popups and right-side selected-object Inspector, with the old duplicate stage panels still guarded as absent.
- UI redesign Phase 9 closure sync now has `0.0.1.8.15.5`: plan, dashboard, work log, UI architecture docs, release notes, tester instructions, project file audit and generated manual are synchronized after the polish/performance/visual/dead-CSS pass; `0.0.1.8.16` now closes the broader visual regression coverage.
- UI redesign visual regression now has `0.0.1.8.16`: fixed viewport attachments cover empty shell/tree/error state, card editor Properties, campaign map popup plus selected-object Inspector, Knowledge Graph context overlay and empty task tracker board across dark/contrast themes and compact/large scale.
- UI redesign owner visual completion is now closed at `Usable` with `0.0.1.8.17`: normal Tools no longer exposes the internal component catalogue, tree rows have stronger keyboard/ARIA semantics, editor controls and empty Task Tracker are quieter, Knowledge Graph first-layer noise is reduced without adding new graph features, campaign map popups avoid the right-side Inspector, and owner evidence screenshots cover 1440x900 plus 1280x720 states.
...
- `0.0.1.11.1` restores persistent workspace switching: the AppShell topbar now keeps a compact `Открыть папку` action while a workspace is active. Cancel leaves the current workspace/page in place, successful A -> B switch reloads B and clears A's open page view, and pending A edits are saved through the existing editor lifecycle before the picker changes workspace.
- `0.0.1.11.2` closes `BI-010` with `NOT REPRODUCED WITH STRONG EVIDENCE`: the current Campaign Map toolbar stayed visible and interactable across map -> card -> map, map A -> map B -> map A, task/rule detours, tree hide/show, presentation return and workspace A -> B map lifecycles. A focused browser evidence guard now locks that matrix.
- `0.0.1.11.3` fixes `BI-011`: the Campaign Map creature token skill action/submenu now renders clean Russian `Навыки`, representative skill labels such as `Скрытность`, and no mojibake markers. `BI-022` is done for the current human-facing P1 regression bundle.
- `0.0.1.11.4` verifies `BUG-001` as acceptable on the current real large workspace: native desktop click-through covers workspace restore, representative card open, Settings diagnostics, tree scroll/search, heavy Campaign Map open and presentation without unexpected runtime errors or resource failures.
...
- `0.0.1.14.0` NF-002 Safe Dice Engine is closed. The final contract includes the safe parser/evaluator, explicit limits, deterministic RNG, immutable structured RollResult, advantage/disadvantage, natural d20 critical metadata, public consumer API and Campaign Map initiative parity. No dice UI, event log, combat functionality or persistent format change was implemented during NF-002.
...
- `0.0.1.15.5` makes Dice Engine `RollResult` the first real durable Event Log consumer: `logDiceRoll()` wraps one public `rollDice()` result in a completed transaction with a `roll.performed` event and appends it to `.my-own-world-events/transactions.v1.jsonl`. The payload preserves formula, faces, total, mode and critical metadata, does not persist parser AST, and failed append does not report durable success.
- `0.0.1.15.6` adds the first stateful event transaction: `logPagePropertyResourceChange()` changes one numeric Properties-backed page field through `PageCommandService`, then appends one completed `resource.changed` transaction. The payload records before/after/delta and a stable page-property resource id. Failed page writes leave no event, and failed event append rolls the page content back through PageCommandService.
- `0.0.1.15.7` adds the first durable undo/reversal path: `undoTransaction()` supports the existing page-property `resource.changed` transaction, refuses roll-only/non-reversible history, blocks double undo from existing reversal records, checks current resource state before compensation, applies the reverse change through `PageCommandService` and appends a new auditable reversal transaction instead of editing or deleting the original history.
- `0.0.1.15.8` adds the public Event Read & Query API: `queryEventLog()` returns bounded typed event items with recent/chronological order, cursor pagination and filters for transaction id, event type, entity id, timestamp range and durable log-order range. `getEventTransactionById()` returns a typed transaction or `null` for unknown ids, so future UI/combat consumers do not parse raw JSONL records.
- `0.0.1.15.0` NF-003 Event / Roll / Combat Log + Transactions is closed. The final gate confirmed one canonical Event Store, durable append/read/reload, RollResult logging, one reversible page-property resource transaction, bounded query API, minimal “Журнал событий” UI and explicit failure/conflict/recovery outcomes. A corrective gate invariant rejects completed durable transactions with no events and prevents the page-property transaction adapter from logging a changed field under an unrelated resource identity. No combat session, damage/HP automation, effects engine, turn/round engine, new storage owner or persistent format migration was implemented.
...
- Latest standalone `npm run test` passed locally with 541 node/static tests.
- Latest `npm run verify` passed locally with encoding/syntax checks, 541 tests, synthetic large-workspace smoke, `git diff --check` and docx zip validation.
...
- `npm run test:browser -- tests/browser/knowledge-graph.spec.mjs` passed locally with 2 focused browser tests covering graph canvas operations, node/connect overlay lifecycle markers, laconic visible-slice status, selected-node relation chips, active/muted edge states, collapsed relationship editing and context-menu anchor correction.
- `npm run test:browser -- tests/browser/popup-drag.spec.mjs` passed locally with 5 focused browser tests for popup drag geometry, including fixed positioning, Command Palette centered transform, opening transform animation, viewport clamp and right/inset normalization. Related checks also passed: `node --test tests\popupPosition.test.mjs`, `tests/browser/popup-lifecycle.spec.mjs`, `tests/browser/app-shell.spec.mjs`, `tests/browser/campaign-map-ui.spec.mjs`, `tests/browser/world-package.spec.mjs` and full `npm run test:browser` with 160 browser tests.
- After the popup drag fix and docs sync, `npm run test`, `npm run ui:polish:audit`, `npm run verify` and `git diff --check` passed locally.
- `npm run test:browser -- tests/browser/task-tracker.spec.mjs` passed locally with 4 focused browser tests covering task tracker persistence, legacy JSON upgrade, the `0.0.1.8.14.1` compact workbench UI surface and the `0.0.1.8.14.2` nested icon-button click regression.
- `npm run test:browser -- tests/browser/app-shell.spec.mjs`, `tests/browser/asset-health.spec.mjs` and `tests/browser/visual-regression.spec.mjs --grep visual-safety-captures-core-surfaces` passed locally for the `0.0.1.8.14.2` Settings maintenance popup, diagnostics/asset markers and visual screenshot guard.
- `npm run test:browser -- tests/browser/app-shell.spec.mjs` and `tests/browser/visual-regression.spec.mjs --grep visual-safety-captures-core-surfaces` passed locally for the `0.0.1.8.14.3` Help/Support/Release guide, including compact Tools route markers, internal help navigation, World Package MVP status, no horizontal overflow and a visual screenshot guard.
- `npm run test:browser -- --grep app-shell-global-workspace-switch-keeps-cancel-and-loads-next-workspace`, full `tests/browser/app-shell.spec.mjs`, `tests/browser/editor-autosave.spec.mjs`, `node --test tests\workspaceStorage.test.mjs`, `node --test tests\storageAdapter.test.mjs --test-name-pattern "getRenderableImageURL.*workspace"`, `npm run verify`, `npm run ui:polish:audit` and final `git diff --check` passed locally for `0.0.1.11.1` Workspace Switch Access.
- `npm run test:browser -- --grep campaign-map-toolbar-survives-page-workspace-and-presentation-lifecycle`, full `tests/browser/campaign-map-ui.spec.mjs`, `tests/browser/editor-autosave.spec.mjs --grep editor-open-page-ignores-stale-async-campaign-map-completion`, `--grep app-shell-global-workspace-switch-keeps-cancel-and-loads-next-workspace`, `tests/browser/visual-regression.spec.mjs --grep visual-safety-captures-core-surfaces` and `npm run verify` passed locally for `0.0.1.11.2` `BI-010` Campaign Map Toolbar.
- `npm run test:browser -- --grep campaign-map-token-skill-menu-renders-russian-labels-and-uses-character-model-checks`, `tests/browser/campaign-map-ui.spec.mjs --grep "campaign-map-(contextmenu-opens-custom-object-actions|token-skill-menu-renders-russian-labels-and-uses-character-model-checks)"`, full `tests/browser/campaign-map-ui.spec.mjs`, `npm run check:encoding` and `npm run verify` passed locally for `0.0.1.11.3` `BI-011` Creature Skills Encoding.
- `npm run desktop:large-workspace-smoke -- --workspace "X:\ДНД\Мастер\По кампаниям\База"` and `node tools\run_desktop_native_clickthrough.mjs --workspace "X:\ДНД\Мастер\По кампаниям\База" --timeout 120000` passed locally for `0.0.1.11.4` `BUG-001` Large Workspace UX. The native pass measured workspace restore 2336 ms, card open 165 ms, Settings diagnostics 718 ms, tree scroll/search 181 ms, heavy map open 36 ms and presentation 1328 ms.
- `npm run desktop:build`, `npm run desktop:native-smoke -- --workspace "X:\ДНД\Мастер\По кампаниям\База"`, `npm run desktop:large-workspace-smoke -- --workspace "X:\ДНД\Мастер\По кампаниям\База"`, `npm run desktop:packaging-smoke`, `npm run desktop:gate -- --workspace "X:\ДНД\Мастер\По кампаниям\База"` and `npm run verify` passed locally for `0.0.1.11.5` `BUG-003` Desktop Real-App Verification. The native pass measured workspace restore 2201 ms, image card open 219 ms, Settings diagnostics 799 ms, tree scroll/search 185 ms, heavy map open 230 ms and presentation 1233 ms; card image render and map background render were both verified.
- `npm run test:browser -- --grep campaign-map-presentation-representative-map-workflow-stays-current`, `npm run test:browser -- tests/browser/campaign-map-presentation.spec.mjs`, visual safety capture, `npm run desktop:build`, native presentation smoke on `X:\ДНД\Мастер\По кампаниям\База`, `npm run verify` and `git diff --check` passed locally for `0.0.1.11.6` `BUG-004` Campaign Map Presentation Stabilization.
- `npm run test:browser -- --grep campaign-map-drawing-tools-stay-usable-through-layers-keyboard-and-reload`, full `tests/browser/campaign-map-ui.spec.mjs`, layer/model/serializer unit checks, visual safety capture, `npm run verify` and `git diff --check` passed locally for `0.0.1.11.7` `BUG-005` Campaign Map Drawing Tools Stabilization.
- `node --test tests/worldPackage.test.mjs` and `npm run test:browser -- tests/browser/world-package.spec.mjs` passed locally for the `0.0.1.8.14.7` World Package manager, including asset payload export, valid-base64 payload guarding, non-overwrite asset copy, imported page asset-reference rewrite, asset-only payload import, Tools entry, branch export, library refresh, default conflict blocking, copy-mode conflict import, parent rewiring, required missing asset blocking, optional missing asset warning, embedded rulePackage file write, page import after backup, sanitized imported HTML and layout guards.
- `npm run test:browser -- tests/browser/campaign-map-ui.spec.mjs` passed locally with 18 browser tests, including the `0.0.1.8.12.10` split toolbar/title/tool-rail guards for full-size zones, no internal scrollbars, unclipped floating tooltip labels and absence of duplicate scene/layer panels, property Inspector and custom right-click guards, the `0.0.1.8.12.2` shared popup-frame guard, the `0.0.1.8.12.6` group visibility action guard, plus current map token, drawing, music, layers, fog and selection flows.
- `npm run test:browser -- tests/browser/campaign-map-data.spec.mjs`, `campaign-map-performance.spec.mjs`, `campaign-map-presentation.spec.mjs` and `campaign-map-initiative.spec.mjs` passed locally after the toolbar, popup and selection-dock migrations.
...
- `npm run test:browser -- tests/browser/visual-regression.spec.mjs` passed locally with 6 browser tests, including the Phase 0 baseline attachments, Phase 9 theme-scale workbench attachments, the `0.0.1.8.16` fixed viewport design-system state matrix and the `0.0.1.8.17` owner evidence matrix.
...
- `npm run ui:polish:audit`, `node --test tests/themeManager.test.mjs`, `npm run test:browser -- tests/browser/app-shell.spec.mjs`, `tests/browser/campaign-map-ui.spec.mjs`, `tests/browser/knowledge-graph.spec.mjs` and `tests/browser/visual-regression.spec.mjs --grep visual-safety-captures-core-surfaces` passed locally for the `0.0.1.8.15.1` contrast/focus/blur polish slice.
- `npm run test:browser -- tests/browser/ui-polish-performance.spec.mjs` passed locally for the `0.0.1.8.15.2` migrated-surface performance smoke.
- `npm run test:browser -- tests/browser/visual-regression.spec.mjs --grep visual-theme-scale-captures-workbench-baselines` passed locally for the `0.0.1.8.15.3` theme-scale visual guard.
- `npm run test:browser -- tests/browser/visual-regression.spec.mjs --grep visual-design-system-captures-fixed-viewport-state-matrix` passed locally for the `0.0.1.8.16` design-system fixed viewport state matrix.
- `npm run test:browser -- tests/browser/visual-regression.spec.mjs --grep visual-owner-completion-captures-primary-secondary-evidence` passed locally for the `0.0.1.8.17` owner visual completion matrix.
- `npm run test:browser -- tests/browser/campaign-map-ui.spec.mjs --grep campaign-map-toolbar-uses-migrated-mode-action-groups`, full `tests/browser/campaign-map-ui.spec.mjs`, `npm run ui:polish:audit`, full `tests/browser/visual-regression.spec.mjs` and `node --test tests/uiMigrationBaselines.test.mjs` passed locally for the `0.0.1.8.15.4` dead CSS cleanup.
- `npm run desktop:gate` passed locally, including docs index, skills validation, verify, 117 browser smoke tests, desktop frontend prepare, packaging smoke, desktop environment check and Tauri cargo check. The large-workspace desktop smoke was skipped because no external workspace path was provided.
