...
## 20.10.1. Реализованный Transport Foundation

Пункт 20.10.1 закрыт как базовый desktop transport:

- добавлена отдельная страница `presentation.html`;
- добавлен entry point `js/presentation/presentationEntry.js`;
- мастерское окно открывает Tauri `WebviewWindow` с label `campaign-map-presentation`;
- синхронизация идет через `BroadcastChannel`;
- в presentation runtime поддержаны независимые zoom/pan;
- popup "Открыть изображение" теперь может показывать картинку и в отдельном presentation window;
- browser fallback через старый `window.open` сохранен.

Ограничение текущей реализации: master пока отправляет HTML snapshot подготовленного presentation clone. Следующий hardening шаг - заменить этот payload на JSON snapshot из `CampaignMapModel` и сделать renderer в presentation window полностью model-first.
