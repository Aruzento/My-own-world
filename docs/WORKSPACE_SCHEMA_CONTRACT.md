...
- добавлены чистые валидаторы workspace/page/map/task;
- validation пока warning-only при загрузке workspace;
- тесты проверяют invalid JSON, missing id, broken parent, duplicated title, broken map token и broken task data.

Следующий этап:

- добавить backup/snapshot перед recovery;
- сделать UI-экран диагностики workspace;
- подключить schema validation к рискованным операциям: delete, move, import, schema upgrade.
