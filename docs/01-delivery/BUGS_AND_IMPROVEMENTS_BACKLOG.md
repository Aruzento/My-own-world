...
| BI-005 | P2 | Knowledge graph | Fix with block | `0.0.1.5.0` | `0.0.1.5.1`-`0.0.1.5.5` add the real canvas foundation, readable workbench layout, filters, right-click node actions, direct node dragging, standard root view, dynamic canvas expansion, readable relationship creation and the large-graph performance gate. Regression coverage remains in `0.0.1.5.6`. |
