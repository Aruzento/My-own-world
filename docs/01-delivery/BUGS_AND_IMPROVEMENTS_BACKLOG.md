...
| BI-022 | P1 | User-visible regression bundle | Ready for plan | `0.0.1.11.0` | PROMOTED TO PROJECT_PLAN PHASE 3. Tracking bundle for current human-facing P1s. `BI-013` block drag-and-drop is closed in `0.0.1.8.11.2`; `BI-010` disappearing map toolbar is not reproduced with strong current evidence in `0.0.1.11.2`; remaining bundle item is `BI-011` creature skills menu encoding. |
