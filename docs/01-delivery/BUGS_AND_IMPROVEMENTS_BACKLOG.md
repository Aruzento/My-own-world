...
| BI-003 | P1 | Campaign map | Fix with block | `0.0.1.11.0` | PROMOTED TO PROJECT_PLAN PHASE 3. Map subsystems should be stabilized before new map features: presentation, fog/drawing/layers, playlists, initiative and regressions. Presentation `BUG-004` is fixed in `0.0.1.11.6`; drawing tools `BUG-005` are verified in `0.0.1.11.7`; map music `BUG-006` is fixed in `0.0.1.11.8`. |
