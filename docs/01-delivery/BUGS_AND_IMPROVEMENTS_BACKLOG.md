...
| BI-017 | P2 | Knowledge graph / maintainability | Ready for plan | `0.0.1.5.x` | Partly advanced in `0.0.1.8.13.5`-`0.0.1.8.13.6`: graph CSS now has separate base/slice/inspector/overlay owners, and selected-node inspector rendering, graph node icon mapping and relationship labels/editable type options moved out of `knowledgeGraphPage.js`. Remaining work: split canvas renderer, toolbar/actions, context menu/overlays and persistence/lifecycle bridge before deeper graph behavior, with regression coverage protecting the split. |
