...
| BI-017 | P2 | Knowledge graph / maintainability | Ready for plan | `0.0.1.5.x` | Partly advanced in `0.0.1.8.13.5`: graph CSS now has separate base, slice, inspector and overlay/menu owners instead of keeping all graph styles in one file. Remaining work: split `knowledgeGraphPage.js` by responsibility before deeper behavior: model/view-state, canvas renderer, toolbar/actions, context menu, inspector and persistence/lifecycle bridge, with regression coverage protecting the split. |
