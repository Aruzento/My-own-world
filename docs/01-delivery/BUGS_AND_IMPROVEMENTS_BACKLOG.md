...
| BI-017 | P2 | Knowledge graph / maintainability | Ready for plan | `0.0.1.5.x` | `0.0.1.8.13.2` made the graph more useful but pushed `knowledgeGraphPage.js` past 6k lines and `styles/knowledge-graph.css` near 2k lines. Before adding more graph behavior, split them by responsibility: model/view-state, canvas renderer, toolbar/actions, context menu, inspector, persistence/lifecycle bridge and styles, with regression coverage protecting the split. |
