...
| BI-016 | P2 | Knowledge graph / operations | Ready for plan | `0.0.1.5.x` | Partly covered in `0.0.1.5.4.3`-`0.0.1.5.5` and `0.0.1.8.13.2`: manual relationships can be edited/deleted from the node context menu, new canvas relationships ask for type/label before saving, and selected nodes now expose visible incoming/outgoing relation inspection in the graph inspector. Remaining future work: richer context actions, relation source/type operations, filters/actions that turn the graph into a daily world tool, and no permanent noisy panels. |
| BI-017 | P2 | Knowledge graph / maintainability | Ready for plan | `0.0.1.5.x` | Partly advanced in `0.0.1.8.13.5`-`0.0.1.8.13.10`: graph CSS now has separate base/slice/inspector/overlay owners; selected-node inspector rendering, graph node icon mapping and relationship labels/editable type options moved out of `knowledgeGraphPage.js`; graph filterbar, view presets, slice-meter helpers, hidden-reason labels and layout-button HTML now live in `knowledgeGraphCanvasControls.js`; graph visible-node fallback, empty state, SVG edges, edge labels and node-card HTML now live in `knowledgeGraphCanvasRenderer.js`; runtime filters, layout/filter/slice actions and zoom/fit toolbar actions now live in `knowledgeGraphCanvasActions.js`; connect-state reads, node/connect popup controllers, node-menu actions and connect-popup actions now live in `knowledgeGraphCanvasOverlays.js`. Remaining work: split relationship-menu/context-menu HTML ownership and persistence/lifecycle bridge before deeper graph behavior, with regression coverage protecting the split. |
