...
- Исправлен сопутствующий баг: шестеренка блока `Свойства` теперь появляется и тогда, когда `applyBlockSystemContract()` применяется прямо к новому блоку, а не ко всему editor.
...
- Импорт package-файла теперь проверяет конфликт rule id и не перезаписывает существующее правило без явного решения пользователя.
...
- Добавлены экспорт и импорт JSON-пакета правил. Импортированные правила получают `sourceType: "rulePackage"`, если источник не был задан явно.
...
- Изменение HP для блока `Свойства` теперь использует `applyCharacterHealthChange()`, то есть математика урона, лечения, временных хитов и `kill/restore` живет в `CharacterModel`.
...
- Инициатива становится производной от карточки, а не отдельным несвязанным числом.
- Изменение хитов на карте постепенно уходит от дублированной математики к единому доменному API.
...
> Статус: архивировано. Подробности перенесены в `docs/02-architecture/ARCHIVED_EXPERIMENTS.md`.
...
- Добавлен архивный документ `docs/02-architecture/ARCHIVED_EXPERIMENTS.md`.
