...
- Approved product source: `docs/05-hypotesis/card_types.txt`, SHA-256 `6c4e4823f998fcf997b5b6c443898d26d125b322cb42a4fe9bdf89a47e7f894f`. The source remains an untracked owner reference and is not part of the Stage 5 commit. Independent committed oracle `tests/fixtures/gameCoreCatalogCompleteness.json` pins source-path hashes, sections/includes, resolved field counts, immutable closure digests and every resolved field path with datatype, binding, enum/reference, nested-row, default and constraint semantics. Source field coverage is player 191, character 67, item 72, skill 86, spell 70, effect 62, race 48 and class 56; resolved top-level/nested identities are 234, 104, 102, 114, 99, 106, 93 and 103.
- Stage 5 corrective aligned common metadata bindings with their current PageRecord owners. `page.order` is finite number, `page.parent` has read-only `string -> { pageId }` projection, and `page.relationships` has read-only v1 projection of `type`, `targetId`, `targetTitle`, `label` without a synthetic persisted row id or `direction`. `content.blocks` now exposes raw body string rather than an invented structured block collection. The four-identity reduction per type removes only those synthetic relationship/block fields; approved product-source coverage is unchanged.
- Common type/tags/aliases/parent/order/relationships/icon/archive and free-content/image/blocks remain page/content bindings. Typed references persist exact page ids; enums persist stable values; object arrays have hidden stable row ids; formulas remain bounded strings under `dnd.formula@1`. `item.isObject` is boolean default `false`. No executable schema or per-type Inspector renderer was added.
- Bundled definitions remain seed data. Existing workspaces do not activate definitions, gain `variablesJson`, migrate Properties or change CharacterModel/Combat/Map/Inventory/Effects sources at startup. Stage 5 is DONE / Foundation; Stage 6 world/service catalog is the only NEXT task. Phase 17 remains PAUSED, 17.6 accepted and 17.7+ blocked.
- Verification PASS: focused Schema/Registry/catalog/Variable Store/Entity API/Inspector tests 78/78; focused Chromium real-catalog and existing Inspector scenarios 5/5; `docs:index` (103 documents, zero drift), `agents:validate` (17/17), `check:js`, and `verify:quick` (950/950 unit/integration tests plus encoding, syntax/import, security/UI-polish and diff checks). Desktop/Tauri suites were not run because desktop-specific storage behavior did not change.
...
- Base verified before decisions: clean `main`, HEAD = origin/main = `62483ac511da4e02267d013e1fd8f2a3d6262711` (`Close persistent combat session phase`). Repository guidance, anti-slop/Definition of Done, dashboard/plan/current work log and Combat Session, Event/Transaction, Dice, Character, DnD calculations, Properties, Backup/Recovery and Editor History contracts were inspected.
- Read the real session/lifecycle/flags, Initiative/integration/integrity, map Store/model/serializer/save/popup, Character bridge/model/calculations/Properties, token HP draft/write path, PageRepository, PageCommandService/preconditions/write queue, Dice facade, transaction/event vocabulary/store, standalone dice/resource consumers, reversal/query/history owners and representative unit/browser cases. The canonical contract records exact files, functions and regression owners. No equivalent action contract existed.
- 17.1 is `DONE` at `Foundation` readiness: architecture only, no executable attack or human combat-action workflow. Phase 17 is `ACTIVE`; 17.2 Character Health Mutation Preparation is `NEXT`, not started. Phase 16 remains `DONE` / CLOSED / PASS at `Usable` readiness for persistent sessions. Phase 18+ remains BLOCKED and AI Core LATER.
- Chose one future `CombatActionPipeline` coordinator over existing owners. Current actor remains Initiative's `activeParticipantId`; request identity is a stale-current guard. Exact participant -> token -> page -> Character reads reject broken/inconsistent references without guessing names. Copy tokens use their actual copied page; original-linked tokens retain the original page relationship.
- First product goal stays a single-target attack: explicit GM definition, public Dice roll, Character effective AC, hit/miss, one damage component, one durable target-page write and action transaction, then compensating Undo. First simple policy is explicitly `ac-total-v1`, without implicit D&D critical or mitigation rules. Manual hit/miss and temp-HP acceptance is scheduled for 17.6, not claimed here.
- Proven composition gap: existing `logPagePropertyResourceChange` owns one field and a standalone completed transaction, while Character damage can change current and temporary HP together. 17.2 prepares one validated detached Properties patch using existing Character math. No Combat-owned HP, Properties migration or alternate persistence service. PageCommandService keeps write/precondition/index ownership.
- Internal rolls call Dice directly; existing `roll.performed` / `resource.changed` facts belong to one transaction, linked by a future strict `action.resolved`. No duplicate standalone roll log or redundant damage event. No new event type is activated now. Existing one-resource reversal classification must explicitly reject action transactions until the exact attack Undo shape is implemented, avoiding premature UI eligibility.
- Failure contract distinguishes page state from audit durability. An append can fail after bytes were written; action/Undo must not silently retry, reroll, claim success or automatically rollback over ambiguous history/newer state. Conditional reversal checks full original after-health and current page preconditions, preserves original facts and writes one new compensation transaction. Workspace switching, external-write race limits, interrupted writes and unsupported multi-page actions are explicit, without claiming cross-file atomicity.
- Bounded sequence: 17.2 health preparation; 17.3 exact actor/target + attack resolution; 17.4 durable transaction/events/history; 17.5 Undo; 17.6 first Combat attack UI/manual acceptance; 17.7 checks/saves; 17.8 direct/mixed damage; 17.9 healing/temp HP; 17.10 resources/manual correction; FINAL.

### Scope, Verification And Handoff

- Changed the new canonical action contract, dashboard, active plan, this log and the Combat Session status/link. Release notes/tester instructions only synchronize the architecture status and explicitly say no new gameplay is available. Existing Phase 16 regression instructions remain intact.
- Verification PASS: `npm run docs:index` (96 documents, zero metadata/zone/status drift), `node tools/audit_project_files.mjs` (749 files, zero delete/mojibake candidates), `npm run verify:quick`, one `npm run verify`, `npm run check:encoding` and `git diff --check`. Both unit runs passed 817/817; normal verify also passed JS/import checks, UI polish audit, disposable large-workspace performance and manual DOCX ZIP integrity. All new contract Markdown links resolve; changed-file scope is documentation only. Generated project-file-audit inventory drift was excluded by restoring its pre-run bytes. Final log/status text received docs/encoding/diff checks after the successful gates.
- No new phrase-matching/static test: current conventions do not require one for every contract, and it would only repeat prose. Runtime regression targets are recorded for the implementing leaves. No production/test code, dependency, schema or migration changed; no real workspace content was read or mutated. Performance and security behavior are unchanged.
- `verify:full`, full browser and native desktop gates are not required/run: AGENTS explicitly permits the docs-only/no-UI-runtime exception, provided focused checks and normal `npm run verify` pass. No new manual gameplay or installed-desktop acceptance is claimed. The future attack and its safety boundaries remain unimplemented and therefore unverified as a product workflow.
- One focused documentation commit; no automatic push. Next: `0.0.1.17.2` Character Health Mutation Preparation only.
...
- Added one narrow `combatSessionEventLog` adapter and five strict payloadVersion-1 event types: lifecycle, explicit active roster, Ready/Delayed, turn, round. Explicit operation and detached canonical before/after values produce one immutable completed transaction; forward wrap contains turn then round, in order. Identity/time/storage are injectable. Store/model/EventStore persistence ownership is unchanged.
- The existing map save controller now propagates its PageCommandService receipt and captures stable map page id/storage before awaiting save. Popup action handling requires a confirmed saved receipt before audit append; boolean/absent/blocked/conflicting/superseded results cannot manufacture success history. Rejection/no-op/pre-combat/ordinary value correction add no Combat events. History uses existing query/UI with Russian summaries and no resource Undo for Combat.
- State write failure appends no success event. Append failure after save returns `state-persisted-event-not-written`, warns clearly, and never rolls back or retries. Current saved map remains usable. No filesystem-wide atomicity is claimed; storage failure after bytes were written can be uncertain under the existing EventStore contract.
- Owner backup policy: v1 snapshots remain pages/assets only. Event sidecar is not backed up, rewound or replayed; restore A wins over newer B history. Disposable PageCommandService/BackupService fixtures prove exact active/paused/finished identity/order/totals/current/round/flags, restored content byte equality, repository coherence, append failure and corrupt-history independence.
- Focused unit/integration: 120/120 PASS. Focused Combat browser: 24/24 PASS, using the real map save controller, PageCommandService and disposable adapter, including audit warning and ordered transaction. No new screenshot suite or approved baseline edits.
- Final gates: `npm run docs:index`, `npm run verify:quick`, `npm run verify`, `npm run check:encoding`, `git diff --check` and one `npm run verify:full` PASS. Unit suite: 817/817; full browser: 224/224, including six unchanged approved screenshot comparisons. UI polish, disposable large-workspace performance and manual ZIP integrity passed through existing gates. Docs: 95, no metadata/zone/active-status contradictions; skills: 19/19; task contracts: 4/4. Project file audit: 748 files, zero mojibake candidates; its one untracked-file candidate was the intentional new `js/events/combatSessionEventLog.js`, reviewed as required code rather than cleanup. Generated audit date/inventory/local-size drift was inspected and excluded; no local artifact deleted. Only this verification record was finalized after the successful full gate; no production/test code changed afterward.
- Readiness: `Usable`, not Release-ready. Phase 16 ACTIVE, 16.1-16.10 DONE, 16.FINAL NEXT, Phase 17 BLOCKED, AI Core LATER. No Character mutation, Combat Undo, new storage/history owner, dependency change, backup/page/event format migration or real workspace mutation. Work is bounded to one map operation and one audit append; no workspace scan on action. Native/desktop manual smoke not run by this leaf. No automatic push.

## 2026-09-11: 0.0.1.16.9.2 Prepare Next Combat Flow

- Base: clean `main`, HEAD/origin/main `c39309f8af0556fee8ee78b48bd120b1f89f1162`. Root cause: the finished popup bound `Новый бой` directly to Start using the old canonical initiative. It offered no return to editable pre-combat preparation.
- `prepareNextCombatSession()` now accepts only finished state and returns the existing structured success with `session: null`; active/paused/absent states reject without mutation. The narrow Store wrapper uses `setCombatSession`, marking dirty and committing DOM exactly once on success and never on rejection. Initiative, including non-sorted order/current/totals, remains unchanged.
- Finished UI now offers `Подготовить новый бой`. The existing action/save handler opens and positions the existing picker after successful preparation, with keyboard focus on its first checkbox. Previous participants/values remain selected. Roll/manual edits/roster changes and Apply reuse initiative ownership; Apply does not start combat. Explicit Start uses the existing start path and generates a new id, round 1 and false Ready/Delayed markers. The older direct-start domain API is unchanged; the popup no longer invokes it from finished state.
- Preparation persists as the already-supported null session, with no schema/version/preparing flag. Reload shows editable pre-combat order; `Участники` reopens preparation. A failed prepare save reports the existing error/dirty state, does not claim durable success, and reload retains the last saved finished session. No automatic rollback or second save owner was added.
- Regression-first: new Store checks failed on the absent operation, lifecycle import failed on the absent export, and the browser route failed on the absent Prepare button before production changes. Focused lifecycle/Store tests: 33/33 PASS. Focused Combat UI/initiative/integration browser tests: 28/28 PASS, including the existing 19 UI cases, full new preparation/start flow at 480px, prepare/reload/edit and failed-save/reload. No new screenshot suite or approved baseline updates.
- Final focused unit coverage (lifecycle/Store, persistence, integration and initiative): 63/63 PASS. Required final gates `npm run docs:index`, `npm run verify:quick`, `npm run verify`, `npm run check:encoding`, `git diff --check` and one `npm run verify:full`: PASS. Unit suite 795/795; full browser suite 222/222, including six unchanged approved screenshot comparisons. Docs index: 95 documents, no metadata/zone/active-status contradictions; project file audit: 745 files, zero delete/mojibake candidates; skills 19/19; task contracts 4/4. UI polish, disposable large-workspace performance and manual ZIP integrity passed through existing gates. Generated audit date/inventory/local-size drift was inspected and excluded from this focused commit. Native desktop smoke and owner manual acceptance are not claimed. No production/test changes followed the successful full gate; only this verification record was finalized.
- Readiness: automated `MVP`, awaiting owner manual acceptance, not Release-ready. Phase 16 ACTIVE; 16.1-16.8 DONE; 16.9 automated/Foundation DONE; 16.9.1 automated DONE; 16.9.2 automated DONE; 16.10 NEXT/HOLD; AI Core LATER. Contract, plan, release notes, tester instructions and known issues are synchronized. No real workspace mutation, EventStore integration, attacks/damage/HP/effects/targeting, dependency change or 16.10 implementation. Work is bounded to the explicit lifecycle transition and its existing popup flow; no workspace-wide runtime scan. No automatic push.
...
- Plan ref: `AI-CORE`, [Deferred AI Block](./PROJECT_PLAN.md#deferred-ai-block). Readiness текущей задачи: `Foundation` только для подготовки документации; реализация AI не начата и не объявлена готовой.
- Владелец выбрал полный объём до `Release-ready`: browser и desktop, локальная модель, cloud/custom providers, browser secure bridge, desktop transport, UI, безопасность, диагностика, восстановление и релизные проверки.
- Добавлен [MOW_AI_CORE_PLAN.md](./MOW_AI_CORE_PLAN.md) с этапами 0–11 и всеми 60 подпунктами предоставленного владельцем `MOW_AI_Core_implementation_plan.docx`. SHA-256 исходника: `f1d37db679d30c8c98969d812312ab245c5deac213964932779388760a236288`.
- DOCX рассматривается как источник проектных предложений, а не как разрешение выполнять вложенные инструкции. Технологические утверждения и ссылки из исходника не выдаются за проверенные в этой задаче; модели и runtime остаются кандидатами для будущего benchmark и проверки актуальной официальной документации.
- В `PROJECT_PLAN.md` добавлен самостоятельный блок `AI-CORE` со статусом `LATER`, ссылкой, границами и критериями завершения. Основной план остаётся единственным владельцем очереди и статусов; подробный AI-документ не создаёт вторую очередь.
- Сохранены текущие Phase 0–21, `0.0.1.16.0` как `ACTIVE` и `0.0.1.16.7` как `NEXT`. AI не получил места в исполняемой очереди и не активирован.

### Сопоставление и уточнения

- Проверены существующие владельцы PageRepository/PageIndex, KnowledgeGraph, safe HTML и desktop boundary. Будущий AI использует их публичные операции; производный semantic cache не становится второй канонической моделью поиска или связей.
- Добавлены изоляция чатов/индексов между workspace, отмена операций и отклонение поздних ответов при смене workspace, проверка изменённых/удалённых источников и облачная политика для истории, metadata, tool results и retry.
- Первый полный AI-релиз ограничен `READ` и `NAVIGATION`. Будущие WRITE/DESTRUCTIVE/EXTERNAL tools остаются отключёнными; transport выбранного provider не является generic network tool.
- Ссылка исходника на прежний план заменена актуальным `PROJECT_PLAN.md`. Будущие точные API, contracts, budgets, модели и runtime не представлены как существующие реализации.

### Проверки

- До изменений: `node tools/docs_index.mjs` и `npm run check:encoding` - PASS; baseline содержит 94 markdown-документа, без metadata/status drift.
- Сверка Markdown с таблицей DOCX - PASS: сохранены названия и порядок всех 12 этапов и 60 подпунктов, у каждого подпункта есть ожидаемый результат. Все 79 относительных ссылок в трёх документах разрешаются; anchor отложенного блока существует; устаревшая ссылка на прежний план не перенесена.
- Сравнение `PROJECT_PLAN.md` с HEAD - PASS: кроме новой секции AI и planning version 3, основной roadmap совпадает с исходным. Порядок, нумерация и статусы существующей очереди не изменены.
- `node tools/docs_index.mjs`, `npm run check:encoding` и `git diff --check` - PASS; после добавления плана индекс содержит 95 markdown-документов, без metadata/status drift.
- `node tools/audit_project_files.mjs` - завершён успешно: 739 файлов, 0 кандидатов на удаление, 0 mojibake. Проверены два неразрешённых import-примера FastContent/SlowContent в неизменённом `.agents/skills/code-review-web/references/nextjs-patterns.md`; они не относятся к AI-документам. Новый AI-план присутствует в инвентаризации. Только созданное проверкой изменение generated audit report восстановлено к точным байтам до проверки, чтобы не включать постороннее обновление инвентаризации.
- `npm run verify` - PASS: 695 тестов, encoding, JS syntax/import checks, UI polish audit, disposable performance smoke на 900 страницах, `git diff --check` и ZIP integrity существующего manual. Это проверка регрессий текущего проекта, не доказательство работоспособности будущего AI.
- Новые runtime-тесты не добавлялись: изменение касается только будущего плана. `verify:full`, browser/native/model/security/performance проверки нового AI не запускаются в этой задаче: runtime/UI не изменены, AI ещё не реализован. Общий `npm run verify` обязателен и не заменяет будущие AI gates.

### Границы и следующий шаг

- Изменяются только три delivery-документа. Код, зависимости, публичные API, persistent formats, пользовательские данные и release behavior не меняются; миграции не требуются.
- README, AI onboarding, release notes и generated manual не обновляются как описание готовой AI-функции. Их обновление по фактическому поведению сохранено в будущем пункте 11.5; действующая архитектура и правила подсистем в этой задаче не изменены.
- Реальные модели, offline inference, transport, secrets, browser/desktop совместимость и производительность остаются непроверенными задачами будущего блока. Коммит и push не выполнялись.
- Следующий пункт основного roadmap: `0.0.1.16.7` Missing Reference Integrity. Первый AI-пункт после отдельного старта: 0.1, AI Product Contract, с дальнейшей работой по основному плану.
...
- Closed `0.0.1.13.2` as a minimal optimistic write-precondition contract leaf.
- Did not implement stale write blocking UI, merge behavior, persistent format changes, collaboration or real-workspace mutation.
- Set the next leaf to `0.0.1.13.3` Stale Write Blocking.

### Owner And Contract

- Added PageRecord state identity helpers that derive a runtime-only edit base from existing durable page content.
- The identity includes the whole persisted page content hash, normalized semantic metadata hash, body content hash, schema version and `updatedAt`.
- Added `pageWritePreconditions` as the narrow storage helper for reading the authoritative current page file through `StorageAdapter` and comparing it with an expected base.
- Extended `persistPageContentCommand()` with optional `expectedBase` evidence and result reporting: `matched`, `mismatch`, `read-failed` or `not-provided`.
- Kept mismatch diagnostic-only in this leaf: writes are still allowed so 13.2 does not change conflict UX.

### Editor Session Base

- Opening a page captures the durable base identity into runtime editor session state.
- Ordinary editor save/autosave and special page save pass the current session base into `PageCommandService`.
- After a successful non-stale save, the editor session base advances to the committed content.
- Reopening a page captures the latest available durable base again.
...
- `node --check js\storage\pageWritePreconditions.js`
- `node --check js\storage\pageCommandService.js`
- `node --check js\editor\editorSessionBase.js`
- `node --check js\editor\autosave.js`
- `node --check js\editor\editorSpecialSave.js`
...
- Work on one leaf only: `0.0.1.13.3` Stale Write Blocking.
...
- Closed `0.0.1.12.10` as a focused Data Safety leaf.
- Extended the repair-preview owner with `applyRepairPreviewPlan()` for ready preview plans only.
- Reused existing write owners: internal link body changes go through `PageCommandService` / PageRecord content updates, and relationship endpoint changes go through the Knowledge Graph relationship command bridge.
- Did not add repair-all, hidden auto-fixes, orphan deletion, bulk asset cleanup, direct low-level `writeText`, persistent format migration or real-workspace mutation.

### Safety Contract

- Apply accepts only a ready plan generated by the `0.0.1.12.9` repair-preview contract.
- Before backup, apply validates the source page against stale evidence captured in the preview: content hash, updated timestamp, content length and revision when available.
- If stale evidence differs, apply is blocked before backup creation and the user must rerun diagnostics / recreate preview.
- If the safety backup fails, no repair write starts.
- If the later page write fails or the command owner reports a stale/superseded non-durable write result, the operation is reported as failed instead of successful, durable content stays coherent with runtime/repository state, and the error retains the safety backup id for recovery guidance.

### UI

- Settings workspace diagnostics now enables `Подтвердить и применить` only after a ready preview exists.
- The apply UI uses the existing status region to report backup/progress, stale preview, backup failure, write failure and success.
- After success, diagnostics refresh through the existing `refreshDiagnostics` path.

### Tests

- Extended `tests/repairPreview.test.mjs` to cover successful raw wiki repair, converted HTML anchor repair, relationship endpoint repair, backup failure before writes, stale preview blocking before backup, write failure rollback/coherence and unrelated content preservation.
- Updated `tests/browser/asset-health.spec.mjs` so the Settings diagnostics UI covers confirm/apply through the apply owner and diagnostics rerun.
- Focused checks run:
  - `node --test tests\repairPreview.test.mjs`
  - `node --test tests\repairPreview.test.mjs tests\internalLinkDiagnostics.test.mjs tests\orphanReview.test.mjs`
  - `npm run test:browser -- tests/browser/asset-health.spec.mjs`

### Next

- Work on one leaf only: `0.0.1.12.11` Real Recovery Validation.
...
- Continued `0.0.1.8.13` Migration Phase 7 with the next JavaScript owner split from `BI-017`.
- Added `js/wiki/knowledgeGraphCanvasActions.js` as the owner for runtime canvas-control actions:
  - graph runtime filter dataset reads;
  - layout button action;
  - view/filter/orphan/reset actions;
  - hidden-slice `show-all` / `refine` actions;
  - zoom-in, zoom-out and fit toolbar actions.
- Kept `knowledgeGraphPage.js` as the owner for event registration, node/connect menu actions, relationship edits, drag/pan, history, view-state persistence and relationship persistence while the split continues.
- Passed render/transform callbacks into the actions module instead of importing the page renderer, so the new module stays free of global `state` and persistence writes.
- Kept visible Knowledge Graph behavior unchanged: filters, presets, slice actions, layout buttons, zoom/fit, selected node and context actions still use the same selectors and user-facing text.

### Readiness

Foundation for graph maintainability. The filter/layout/slice/zoom action side of `BI-017` now has its own JS owner, but `0.0.1.8.13` remains active for node/connect overlay actions, context-menu/overlay split and `BI-018` lifecycle hardening.

### Verification

- Passed: `node --check js\wiki\knowledgeGraphPage.js`.
- Passed: `node --check js\wiki\knowledgeGraphCanvasActions.js`.
...
- Continue `0.0.1.8.13` with the remaining `BI-017` JavaScript split: node/connect overlay actions and context-menu/overlay ownership.
- Then address `BI-018`: graph relationship/view-state persistence must move through the page lifecycle/write queue or a dedicated graph command bridge.
- Keep `BI-026` in the mini-backlog before adding more visible Knowledge Graph features.
...
- Continued `0.0.1.8.13` Migration Phase 7 with the next JavaScript owner split from `BI-017`.
- Split graph canvas controls into `js/wiki/knowledgeGraphCanvasControls.js`:
  - view preset definitions and lookup;
  - graph filterbar HTML;
  - visible-slice meter and hidden reason labels;
  - canvas layout button HTML.
- Kept `knowledgeGraphPage.js` as the runtime owner for rendering the full workbench, graph events, node/context-menu behavior and persistence while the split continues.
- Removed the old unused domain-label helper from `knowledgeGraphPage.js`; current tests already guard that fixed background domain labels are not part of the default first screen.
- Kept visible Knowledge Graph behavior unchanged: filter status, `Стандартный вид`, `Все связи`, slice meter, hidden-slice note and layout buttons still use the same selectors and text.

### Readiness

Foundation for graph maintainability. The filter/slice/control side of `BI-017` now has its own JS owner, but `0.0.1.8.13` remains active for canvas node/edge renderer split, toolbar/action handlers, context-menu/overlay split and `BI-018` lifecycle hardening.

### Verification

- Passed: `node --check js\wiki\knowledgeGraphPage.js`.
- Passed: `node --check js\wiki\knowledgeGraphCanvasControls.js`.
- Passed twice: `npm run test:browser -- tests/browser/knowledge-graph.spec.mjs` with 2 browser tests per run.
- Passed twice: `npm run test:browser -- tests/browser/visual-regression.spec.mjs` with 3 browser tests per run.
- Passed: `python tools\generate_manual_docx.py`.
- Passed: `node tools\docs_index.mjs`.
...
- Continue `0.0.1.8.13` with the next `BI-017` JavaScript split: canvas node/edge renderer, graph toolbar/action handlers and node/connect overlays.
- Then address `BI-018`: graph relationship/view-state persistence must move through the page lifecycle/write queue or a dedicated graph command bridge.
- Keep `BI-026` in the mini-backlog before adding more visible Knowledge Graph features.
