...
Current leaf: CTV Stage 3 `Variable Store и Variables / Entity API` is `DONE` at `Foundation` readiness. Canonical design: [CARD_TYPES_VARIABLES_MIGRATION.md](../02-architecture/CARD_TYPES_VARIABLES_MIGRATION.md). CTV Stages 1–2 remain `DONE` / `Foundation`; CTV Stage 4 `Universal Inspector` is the only `NEXT` migration task and requires a separate task. By the current owner instruction on 2026-09-24, `0.0.1.17.6` First Combat Attack Workflow remains the last completed and accepted Combat baseline. After the nine-stage migration project and 17.6 parity gate, resume at 17.7. Phase 16 remains CLOSED / PASS; Phase 18+ BLOCKED; AI Core LATER.
...
| CTV Stage 4 — `Universal Inspector` | `NEXT` — separate task required | One schema-driven Inspector and generic field components; no type-specific Inspector families. |
| CTV Stage 5 — `Каталог типов — игровое ядро` | `BLOCKED` on Stage 4 | Player, Character, Item, Skill, Spell, Effect, Race and Class schemas/field catalogues. |
