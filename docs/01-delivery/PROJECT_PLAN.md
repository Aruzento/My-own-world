...
Статус: **сделано MVP+hotfix**. Поля `Свойств` получили встроенный compact grip для pointer-based перестановки, runtime-placeholder для красивого сдвига соседей, удаление видимых стандартных и пользовательских полей через popup шестеренки, выбор готовых параметров при создании поля и динамический resize через восемь точек по сторонам/углам. Drag можно завершить в пустую область сетки блока, кнопка размера убрана из шестеренки как рудимент, а левый/верхний resize меняет именно выбранную сторону поля. Runtime grip/resize больше не являются кнопками, чтобы не перехватывать клик по текстовым полям внутри label. Размер хранится как persistent `data-property-span` / `data-property-rows` в 12-колоночной сетке. Полные абсолютные `x/y`, группы и более глубокая model-first layout-сетка остаются будущим расширением.
