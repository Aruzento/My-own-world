...
Current leaf: CTV Stage 5 `Каталог типов — игровое ядро` is `DONE` at `Foundation` readiness. Canonical design: [CARD_TYPES_VARIABLES_MIGRATION.md](../02-architecture/CARD_TYPES_VARIABLES_MIGRATION.md). CTV Stages 1–4 remain `DONE` / `Foundation`; CTV Stage 6 `Каталог типов — мир и служебные сущности` is the only `NEXT` migration task and requires a separate task. By the current owner instruction on 2026-09-24, `0.0.1.17.6` First Combat Attack Workflow remains the last completed and accepted Combat baseline. After the nine-stage migration project and 17.6 parity gate, resume at 17.7. Phase 16 remains CLOSED / PASS; Phase 18+ BLOCKED; AI Core LATER.
...
- `0.0.1.15.7` Transaction Undo / Reversal - `DONE` on 2026-08-27. Added `js/events/transactionReversal.js` as the first durable undo orchestration layer. V1 undo supports only the proven reversible `resource.changed` page-property transaction from `0.0.1.15.6`: it reads the event log, refuses roll-only/non-reversible transactions, blocks double undo by detecting already appended reversal history, verifies the current numeric resource still matches the original `after` value, applies the compensating `after -> before` change through `PageCommandService`, and appends a new transaction with `reversesTransactionId`, a reversing `resource.changed` event and `transaction.reversal.recorded` metadata. The original transaction/event remains untouched and readable. Failed compensation writes leave no reversal event; failed event append rolls the compensation page write back through the existing page command owner. No UI, combat/session mechanics, HP automation, generic object mutation engine, persistent format migration or real workspace mutation was implemented.
- `0.0.1.15.8` Event Read & Query API - `DONE` on 2026-08-27. Added `js/events/eventQuery.js` as the stable public read facade over the existing durable event store. `queryEventLog()` returns bounded event-query items rather than raw JSONL records, defaults to recent-first order, supports `limit` plus cursor pagination, chronological order, transaction id, event type, target/entity id from supported payload references, event timestamp range and durable log-order range. `getEventTransactionById()` returns a typed transaction by id or `null` for unknown ids. The API reads through `readTransactionRecords()` and does not write, parse storage records in consumers, create UI, add combat/session behavior or mutate workspace data.
...
- `0.0.1.16.2` Combat Session Model - `DONE` at `Foundation` readiness on 2026-09-04. Added the isolated `CombatSessionModel` as the sole domain owner of session identity, lifecycle representation, round, roster membership and typed `ready` / `delayed` flags. Participants retain only the canonical initiative `participantId`; Character snapshots, initiative order/roll/modifier/current-turn state and arbitrary data bags are excluded. Structured missing-reference integrity remains runtime-only and keeps unresolved participant identities without becoming stale persisted diagnostics. No Campaign Map persistence, lifecycle transitions, initiative synchronization, turn/round progression, UI or EventStore integration was implemented.
...
- `0.0.1.16.8` Ready / Delayed / Temporary Flags - `DONE` at `Foundation` readiness on 2026-09-10. Added pure `setCombatParticipantFlags(currentSession, participantId, patch)` in `js/combat/combatSessionFlags.js`, depending only on CombatSessionModel, and a narrow same-named CampaignMapStore method. Explicit patches accept only non-empty boolean `ready` / `delayed` fields for an exact existing Combat member in an active session; paused/finished/inactive/missing sessions and malformed commands reject without publication. The independent markers may both be true and have no action/reaction, trigger, turn skip or initiative reorder semantics. Unresolved initiative/token/page references do not block Combat-local edits and are never repaired. A changed canonical detached snapshot uses the existing setter for one dirty mark and one DOM commit; an idempotent request publishes nothing. Start/new-session defaults, lifecycle/progression/initiative-edit preservation, explicit roster reconciliation and exact serialize/reload behavior are covered. Persistent participant shape remains only `participantId`, `ready`, `delayed`; integrity stays runtime-only. No schema/version/migration, Character, EventStore, direct page write or UI change; UI wiring remains 16.9.
...
7. `0.0.1.16.8` Ready / Delayed / Temporary Flags - `DONE` at `Foundation` readiness on 2026-09-10.
8. `0.0.1.16.9` Combat UI + Reload Workflow - `DONE` at automated/`Foundation` readiness on 2026-09-10. Existing Initiative popup now derives pre-combat/active/paused/finished UI from canonical state, with explicit Start, Store-owned progression/lifecycle/markers, active-only roster reconciliation and read-only integrity warnings. Active/paused/finished serialize/reload, pending-input rejection safety, one successful action/one save and zero rejected/no-op saves are covered. No second Combat surface/state owner, persistent format change, repair, EventStore or later combat mechanics. Owner manual acceptance is now PASS as sufficient foundation for 16.10; this is not a release-ready claim.
...
ARCHITECTURE: `CombatActionPipeline` coordinates existing Initiative/current participant, exact token/page reads, CharacterModel, pure Dice, Properties preparation, PageCommandService and EventStore. First policy is explicit `ac-total-v1`, with no automatic D&D critical/mitigation rules. One strict `action.resolved` v1 fact links existing `roll.performed` and `resource.changed` facts; it is activated by 17.4. The existing Transaction Reversal owner now supports exact single-page attack compensation through the shared target queue and current health/base guards.

PREREQUISITE: existing resource logging owns one numeric field and its own completed transaction; Character damage can change both `hpTemp` and `hpCurrent`. 17.2 therefore prepares one validated detached Properties patch before action execution. It is not a new HP model, persistence format or substitute product goal. 17.4 proves the existing workspace/precondition/write boundary and explicitly blocks premature resource Undo for action transactions.
...
| `0.0.1.17.2` Character Health Mutation Preparation | `DONE` / `Foundation` | Strict existing Character/Creature Properties health source, one frozen detached current/temp HP patch, exact base/before/after/guards, unchanged state and parser/browser readback; no action runner/write/UI. |
...
| `0.0.1.17.6` First Combat Attack Workflow | `DONE` / accepted baseline | Accepted by the current owner instruction on 2026-09-24. Preserve attack/target/result/history/Undo/reload behavior throughout CTV; no new manual run claimed in CTV Stage 1. |
...
| --- | --- | --- |
| CTV Stage 1 — `Архитектура новой системы типов` | `DONE` / `Foundation` | Repository owners/data-flow audit; canonical PageRecord variablesJson design; metadata separation; Type Registry, Schema, Field Sets, Variable Store, Variables API, resolver and Inspector boundaries; schema/recovery lifecycle; approved legacy type mapping. Canonical architecture/migration design. Documentation only. |
| CTV Stage 2 — `Schema Engine, Registry и Field Sets` | `DONE` / `Foundation` | Production data-only definition validation, exact-version Field Set DAG composition, immutable SHA-256 definition closure identity, resolved Type Registry and optimistic activated-definition workspace catalog. No Variable Store, Inspector, card migration or domain cutover. |
| CTV Stage 3 — `Variable Store и Variables / Entity API` | `DONE` / `Foundation` | PageRecord v2 variablesJson codec; detached typed Store/Entity API; lazy defaults/overrides/references; trusted resolver foundation; immutable plans and existing PageCommand whole-page guards/readback. Explicit safety blocks for legacy portability and asset cleanup. No Properties migration or domain cutover. |
| CTV Stage 4 — `Universal Inspector` | `DONE` / `Foundation` | One schema-driven Inspector in the standard card right panel; generic datatype components, ephemeral validated draft, Entity API reads, Stage 3 guarded commits, explicit source-mode/conflict UX. No type-specific Inspector families or production type catalog. |
| CTV Stage 5 — `Каталог типов — игровое ядро` | `DONE` / `Foundation` | Immutable bundled v1 definitions for Player, Character, Item, Skill, Spell, Effect, Race and Class; four semantic Field Sets; approved-source provenance/completeness oracle; activation/Registry/Inspector evidence. No workspace activation, card migration or domain cutover. |
| CTV Stage 6 — `Каталог типов — мир и служебные сущности` | `NEXT` — separate task required | Location, Region, Country, Organization, Lore, Folder and Project schemas/field catalogues. |
| CTV Stage 7 — `Безопасная миграция Properties` | `BLOCKED` on Stage 6 | Extraction/mapping, preview, backup/recovery, idempotent migration, durable readback and compatibility reader. |
| CTV Stage 8 — `Перевод доменных систем на Variables API` | `BLOCKED` on Stage 7 | CharacterModel, calculations, Combat, Campaign Map, Inventory, Effects compatibility and other domain consumers. |
| CTV Stage 9 — `Удаление legacy active architecture и финальная интеграция` | `BLOCKED` on Stage 8 | Retire Properties as active source; compatibility/retirement, type switching, PageRepository/PageIndex/search/tree, backup/restore, World Packages, templates/duplication, browser/Tauri parity, documentation and 17.6 parity gate. Unblock 17.7 only through a separate task after closure. |
