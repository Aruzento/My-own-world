...
Current leaf: CTV Stage 4 `Universal Inspector` is `DONE` at `Foundation` readiness. Canonical design: [CARD_TYPES_VARIABLES_MIGRATION.md](../02-architecture/CARD_TYPES_VARIABLES_MIGRATION.md). CTV Stages 1–3 remain `DONE` / `Foundation`; CTV Stage 5 `Каталог типов — игровое ядро` is the only `NEXT` migration task and requires a separate task. By the current owner instruction on 2026-09-24, `0.0.1.17.6` First Combat Attack Workflow remains the last completed and accepted Combat baseline. After the nine-stage migration project and 17.6 parity gate, resume at 17.7. Phase 16 remains CLOSED / PASS; Phase 18+ BLOCKED; AI Core LATER.
...
| CTV Stage 5 — `Каталог типов — игровое ядро` | `NEXT` — separate task required | Player, Character, Item, Skill, Spell, Effect, Race and Class schemas/field catalogues. |
| CTV Stage 6 — `Каталог типов — мир и служебные сущности` | `BLOCKED` on Stage 5 | Location, Region, Country, Organization, Lore, Folder and Project schemas/field catalogues. |
