...
Current leaf: CTV Stage 1 `Архитектура новой системы типов` is `DONE` at `Foundation` readiness. Canonical design: [CARD_TYPES_VARIABLES_MIGRATION.md](../02-architecture/CARD_TYPES_VARIABLES_MIGRATION.md). By the current owner instruction on 2026-09-24, `0.0.1.17.6` First Combat Attack Workflow is the last completed and accepted Combat baseline, superseding the earlier not-accepted status. No new manual test is claimed by this documentation stage. CTV Stage 2 `Schema Engine, Registry и Field Sets` is the only `NEXT` migration task and requires a separate task; do not implement it automatically. After the nine-stage migration project and 17.6 parity gate, resume at 17.7. Phase 16 remains CLOSED / PASS; Phase 18+ BLOCKED; AI Core LATER.
...
| CTV Stage 2 — `Schema Engine, Registry и Field Sets` | `NEXT` — separate task required | Declarative schemas, Type Registry, reusable Field Sets, schema validation, composition and versioning. No Variable Store, Inspector, catalog migration or domain cutover. |
| CTV Stage 3 — `Variable Store и Variables / Entity API` | `BLOCKED` on Stage 2 | variablesJson codec; Variable Store; Variables / Entity API; PageRecord persistence/write boundaries; computed resolver foundation. |
| CTV Stage 4 — `Universal Inspector` | `BLOCKED` on Stage 3 | One schema-driven Inspector and generic field components; no type-specific Inspector families. |
