...
0.0.0.1.14.7. Добавить полноценный редактор условий применения правила.

Зачем: foundation уже хранит `conditions`, но пока не дает удобный UI для сборки условий из level/state/card-variable/formula.

0.0.0.1.14.8. Добавить Rule Package import/export и предпросмотр итоговых эффектов правил.

Зачем: после структуры дерева нужно уметь переносить наборы правил между мирами и видеть, какие эффекты реально применятся к персонажу.
