...
Current leaf: `0.0.1.16.9.1` Combat UI Usability Fix is `DONE` automated, awaiting owner manual retest. `0.0.1.16.9` remains `DONE` automated/Foundation, but its owner manual acceptance `FAILED` with a realistic roster. `0.0.1.16.10` Persistence / Recovery / Event Integration is `NEXT`, on `HOLD` until owner acceptance of the corrected UI; not started. AI Core remains `LATER`.
...
9. `0.0.1.16.10` Persistence / Recovery / Event Integration - `NEXT`, `HOLD` pending owner manual retest after 16.9.1; not started.
