...
Current leaf: CTV Stage 6 `Каталог типов — мир и служебные сущности` is `DONE` at `Foundation` readiness. Canonical design: [CARD_TYPES_VARIABLES_MIGRATION.md](../02-architecture/CARD_TYPES_VARIABLES_MIGRATION.md). CTV Stages 1–5 remain `DONE` / `Foundation`; CTV Stage 7 `Безопасная миграция Properties` is the only `NEXT` migration task and requires a separate task. By the current owner instruction on 2026-09-24, `0.0.1.17.6` First Combat Attack Workflow remains the last completed and accepted Combat baseline. After the nine-stage migration project and 17.6 parity gate, resume at 17.7. Phase 16 remains CLOSED / PASS; Phase 18+ BLOCKED; AI Core LATER.
...
| CTV Stage 7 — `Безопасная миграция Properties` | `NEXT` — separate task required | Extraction/mapping, preview, backup/recovery, idempotent migration, durable readback and compatibility reader. |
