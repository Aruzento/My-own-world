...
| E. New accessibility labels contain English strings inside the Russian product. | CONFIRMED | `js/tree/tree.js:158`-`js/tree/tree.js:159` sets `aria-label="World tree"`; `js/tree/treeRender.js:134` uses fallback `Untitled page`; `js/tree/treeRender.js:188`-`js/tree/treeRender.js:189` builds `Expand/Collapse` labels; `js/tree/treeRender.js:367`-`js/tree/treeRender.js:372` builds `Page actions: ...`; `js/taskTracker/taskTrackerColumnHTML.js:87` sets `aria-label="Empty column"`; `tests/browser/app-shell.spec.mjs:1098` and `tests/browser/app-shell.spec.mjs:1152` assert English labels. | `0.0.1.8.18.4` |
| F. Tree semantics improved, but complete ARIA tree keyboard evidence is missing. | PARTIALLY CONFIRMED | `js/tree/tree.js:153`-`js/tree/tree.js:159` sets `role="tree"` and a tree label; `js/tree/treeRender.js:137`-`js/tree/treeRender.js:199` sets `role="treeitem"` / `aria-expanded`; `js/tree/treeRender.js:270` sets `aria-current="page"` for the current page; `tests/browser/app-shell.spec.mjs:1057`-`tests/browser/app-shell.spec.mjs:1164` verifies semantics and Enter-to-open. However `rg` found no `ArrowUp`, `ArrowDown`, `ArrowLeft`, `ArrowRight`, `Home`, `End`, or `keydown` tree contract implementation in `js/tree`, and `js/tree/treeKeys.js:1`-`js/tree/treeKeys.js:19` only returns identity keys for persisted expansion. | `0.0.1.8.18.4` |
| G. Campaign Map has local geometry helpers while shared popup positioning already owns viewport positioning. | CONFIRMED | `js/editor/campaignMapPopupController.js:148` calls feature-local `avoidMapInspectorOverlap`; `js/editor/campaignMapPopupController.js:239`-`js/editor/campaignMapPopupController.js:320` defines local overlap, visibility and clamp helpers; `js/ui/popupPosition.js:45`, `js/ui/popupPosition.js:110` and `js/ui/popupPosition.js:155` already own generic popup viewport clamp behavior. | `0.0.1.8.18.5` |
