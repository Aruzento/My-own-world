...
Next check: continue Phase `0.0.1.12.0` with disposable broken workspace fixtures for broken link diagnostics, staged repair preview and persistent map/task repair flows.

Regression target: storage/browser tests for restore preview, partial restore, restore failure safety, asset verification, link cleanup and future persistent repair actions.
