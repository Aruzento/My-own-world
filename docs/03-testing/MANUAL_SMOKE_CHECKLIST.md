...
15. Create a map and confirm the toolbar has four readable groups: `Создание`, `Сцена`, `Инструменты` and `Live`. At normal desktop width no group should be clipped; grid, pan, drawing and fog buttons should show a clear active pressed state when toggled. Select a token or shape and confirm the bottom selection dock shows identity, position, visibility and available stats, with compact local-icon actions; `Убрать` should remove from the map only, not delete the card. Open add, grid, drawing, fog, layers, music and initiative popups; each should have a compact dark shared frame with an icon, readable title, section labels, no clipping/overlap, and normal Escape/repeated-trigger close behavior.
