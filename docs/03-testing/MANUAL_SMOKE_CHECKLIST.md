...
16. Create/open `Граф связей`; confirm the filterbar shows `показано`, `в мире`, `скрыто`, the status reads like `показано X из Y`, hidden-slice notes explain why nodes are hidden, and `Все связи` / `Уточнить поиск` behave clearly. Click a node and confirm its lines become active, unrelated visible content becomes muted, the graph inspector shows incoming/outgoing relationships and `Открыть` / `Соседи`. Right-click a node and confirm the custom dark node menu opens near the clicked node, stays inside the viewport, groups actions by purpose, shows editable manual relationship rows without clipped fields or visible internal scrollbars, and closes by Escape/outside click. Create a connection through the connect popup and confirm it uses the same dark icon/header/field language.
