...
Tree keyboard check after step 13: focus a tree row and use `Ctrl+Shift+ArrowUp/Down/Right/Left` to reorder/promote/move into the previous visible row. The focused row should stay focused, pointer-drag previews should not appear, and the statusbar should announce the result.
...
Map popup check after step 15: with the right-side Inspector visible, open Grid or another map popup and confirm the popup shifts away from the Inspector instead of covering it or letting Inspector text visually bleed through.
...
20. Create a task tracker and add one task. Focus a task grip and use `Ctrl+Shift+ArrowUp/Down/Left/Right`; focus a column grip and use `Ctrl+Shift+ArrowLeft/Right`. The order should change, focus should remain on the moved handle, and the statusbar should announce the result.
