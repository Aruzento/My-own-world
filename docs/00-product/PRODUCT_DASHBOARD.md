...
- Design-track pause: `0.0.1.8.16` is closed at `Foundation`, and the current `0.0.1.8` redesign plan has no next implementation item. Before moving back to non-design product work, ask the owner for the pending post-design task. Do not add new visible Knowledge Graph features until `BI-026` rethinks the graph concept.
