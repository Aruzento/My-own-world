...
- Continue with `0.0.1.8.15` Migration Phase 9 polish and cleanup: the first accessibility/theme/motion gate is in place, but deeper performance review, dead CSS removal, broader visual regression review and final documentation sync remain. Do not add new visible Knowledge Graph features until `BI-026` rethinks the graph concept.
