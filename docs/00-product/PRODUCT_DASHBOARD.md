...
3. `RCB-021` pending autosave loss, `RCB-001` page rollback/repository consistency, `RCB-001B` metadata index consistency, `RCB-002` durable tree batch rollback and `RCB-003` native desktop smoke failure semantics are closed. Next recommendation is `RCB-022`, then `RCB-004`.
...
- Choose the next `0.0.1.10.0` RCB leaf. Recommended next leaf: `RCB-022`.
