...
`0.0.1.8.14.4` update: the World Package baseline now includes `#worldPackagePopup[data-world-package-ui-migration="0.0.1.8.14.4"]` opened from Tools. The manager is a real MVP surface for branch/world export, saved package library, JSON import preview and backup-gated page-only import. Asset and rulePackage contents remain preview blockers until their apply workflows are implemented.
...
| `visual-world-packages.png` | World Package manager | `#worldPackagePopup[data-world-package-ui-migration="0.0.1.8.14.4"]` opened through Tools -> Пакеты мира | Captures the MVP import/export manager with export controls, package library, import JSON preview, backup/apply state, local sprite icons, data-safety markers and compact two-column layout. |
...
| World Package manager `0.0.1.8.14.4` | `#worldPackagePopup[data-world-package-ui-migration="0.0.1.8.14.4"]`, `[data-world-package-tool-action]`, `[data-world-package-section]`, `[data-world-package-preview]`, `[data-world-package-apply-state]`, `[data-world-package-file]` | `styles/world-package.css` owns the manager popup; `js/ui/worldPackageManager.js` owns rendering and user actions; `js/worldPackage/worldPackageImportService.js` owns backup-gated page-only apply | Local sprite via `iconSvg` for export/library/import/status/action rows | PopupManager owns dialog lifecycle; apply is blocked until preview is valid, conflicts are absent and a backup manifest is created. Asset/rulePackage contents stay blockers until future apply services exist. | VS Code import/export task panels, Notion template/library preview, backup-first data tools. |
