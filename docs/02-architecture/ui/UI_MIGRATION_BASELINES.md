...
| Knowledge graph and canvas | `.knowledge-graph-document`, `.knowledge-graph-workbench[data-knowledge-graph-migration="phase-7-slice"]`, `.knowledge-graph-canvas-stage`, `.knowledge-graph-canvas-slice-stats`, `.knowledge-graph-canvas-slice-note`, `.knowledge-graph-canvas-inspector`, `.knowledge-graph-node-menu[data-knowledge-graph-overlay-ui="0.0.1.8.13.3"]`, `.knowledge-graph-connect-popup[data-knowledge-graph-overlay-ui="0.0.1.8.13.3"]`, node cards and edges | `styles/knowledge-graph.css`; `js/wiki/knowledgeGraph.js` owns slice counts; `js/wiki/knowledgeGraphPage.js` owns runtime graph rendering | Graph toolbar, node metadata, node-menu actions and connect-popup actions use local sprite icons for layout/history/fit/refresh/domain/action hints; no external icon package | Node menu and connect popup register through `popupManager`; the node menu is a context-menu overlay with grouped actions and compact editable manual relationships; the slice note and filterbar counters are runtime canvas controls, not persisted page HTML | Obsidian Graph/Canvas and Miro frames: filters, node/edge tokens, grouping and navigation without becoming a separate whiteboard. |
...
- Knowledge Graph file/CSS split and lifecycle bridge work during the remaining `0.0.1.8.13` slices; visible-slice clarity is migrated in `0.0.1.8.13.1`, selected-node edge states/inspector are migrated in `0.0.1.8.13.2`, and node/connect overlay visual cleanup is migrated in `0.0.1.8.13.3`.
