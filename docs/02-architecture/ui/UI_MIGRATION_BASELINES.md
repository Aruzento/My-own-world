...
`0.0.1.8.13.8` update: the Knowledge graph JavaScript baseline now includes `js/wiki/knowledgeGraphCanvasRenderer.js` for visible-node fallback, canvas empty state, SVG edges, edge labels and node-card HTML. The renderer consumes the canvas model/connect state and remains event-free; `knowledgeGraphPage.js` still owns toolbar/actions, overlays and persistence until the remaining `BI-017` / `BI-018` work.
