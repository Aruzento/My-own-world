...
- Knowledge Graph file/CSS split and lifecycle bridge work during the remaining `0.0.1.8.13` slices; visible-slice clarity is migrated in `0.0.1.8.13.1`, selected-node edge states/inspector are migrated in `0.0.1.8.13.2`, node/connect overlay visual cleanup is migrated in `0.0.1.8.13.3`, laconic first-layer correction is migrated in `0.0.1.8.13.4`, CSS ownership split is migrated in `0.0.1.8.13.5`, and the first JavaScript owner splits now cover inspector, labels, controls, renderer, canvas actions and canvas overlays through `0.0.1.8.13.10`.
