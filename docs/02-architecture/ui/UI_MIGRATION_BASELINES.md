...
Status: `0.0.1.8.10` AppShell migration closed and corrected by `0.0.1.8.10.1`-`0.0.1.8.10.4`; `0.0.1.8.11` core content is closed at `Usable` level by `0.0.1.8.11.7`; `0.0.1.8.12` campaign map migration is active with the toolbar foundation closed by `0.0.1.8.12.1`, the popup surface closed by `0.0.1.8.12.2` and the selected-object inspector dock closed by `0.0.1.8.12.3`.
...
| `visual-campaign-map.png` | Campaign map | `.campaign-map-document` with synthetic tokens, shape, selected-object dock and locked fog zone | Captures map stage, grid, tokens, hidden/player badge, selection inspector dock, shape and fog layering. |
...
| Campaign map and live scene | `.campaign-map-document`, `.campaign-map-stage`, `.campaign-map-controls[data-map-ui-migration="0.0.1.8.12.1"]`, `.campaign-map-control-group[data-map-control-group]`, `#campaignMapPopup[data-map-popup-ui-migration="0.0.1.8.12.2"]`, `.campaign-map-popup-shell[data-map-popup-ui-migration="0.0.1.8.12.2"]`, `.campaign-map-popup-section[data-map-popup-section]`, `.campaign-map-selection-dock[data-map-selection-ui-migration="0.0.1.8.12.3"]`, tokens, shapes, fog canvas | `styles/design-tokens.css` owns `--mow-map-*`; `styles/campaign-map-layout.css`, `styles/campaign-map-responsive.css`, `styles/campaign-map-popups.css`, `styles/campaign-map-selection-inspector.css`, `styles/campaign-map-initiative.css` consume it | Toolbar, popup headers and the selection dock use local sprite icons through `iconSvg` for add, character, creature, object, pan, grid, map image, layers, shapes, drawing, fog, presentation, initiative, music, visibility, duplicate, more and remove actions; do not add external icon packs | Generic `#campaignMapPopup` popups register as modal dialog overlays through `campaignMapPopupController` and now share the migrated map popup frame; `#campaignTokenPopup` registers as a non-modal popover and keeps delayed hover behavior local; the selection dock is a runtime stage surface, not a persisted overlay | Foundry scenes, Roll20 lighting, Owlbear scene/fog clarity. |
...
- Remaining campaign map layer/token dock depth, map inspector surfaces and advanced contextual visual polish during campaign map migration.
