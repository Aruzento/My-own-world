...
`0.0.1.8.12.1` update: the Campaign map baseline now includes `.campaign-map-controls[data-map-ui-migration="0.0.1.8.12.1"]`. The toolbar is an accessible `role="toolbar"` surface with creation, scene, tools and live-session groups, local sprite icons, readable short labels, shared-token styling and `aria-pressed` active states for grid/pan/drawing/fog. Visual smoke keeps using `visual-campaign-map`, and `campaign-map-ui.spec.mjs` guards the toolbar structure and right-edge fit inside the editor workspace.
...
`0.0.1.8.12.3` update: the Campaign map baseline now includes `.campaign-map-selection-dock[data-map-selection-ui-migration="0.0.1.8.12.3"]`. The dock is runtime-only inside `.campaign-map-stage`, updates from map selection events, shows the selected token/shape identity, position, presentation visibility and available token stats, and exposes local sprite contextual actions. The `Убрать` action removes selected map objects from the current map only; it must not delete the linked card/page. `campaign-map-ui.spec.mjs` guards the dock marker, stats, visibility toggle and safe remove path, and `visual-campaign-map` now captures a selected token with the dock visible.
...
| Campaign map and live scene | `.campaign-map-document`, `.campaign-map-stage`, `.campaign-map-controls[data-map-ui-migration="0.0.1.8.12.1"]`, `.campaign-map-control-group[data-map-control-group]`, `#campaignMapPopup[data-map-popup-ui-migration="0.0.1.8.12.2"]`, `.campaign-map-popup-shell[data-map-popup-ui-migration="0.0.1.8.12.2"]`, `.campaign-map-popup-section[data-map-popup-section]`, `.campaign-map-selection-dock[data-map-selection-ui-migration="0.0.1.8.12.3"]`, `.campaign-map-layer-dock[data-map-layer-dock-ui-migration="0.0.1.8.12.4"]`, `.campaign-map-scene-inspector[data-map-scene-inspector-ui-migration="0.0.1.8.12.5"]`, tokens, shapes, fog canvas | `styles/design-tokens.css` owns `--mow-map-*`; `styles/campaign-map-layout.css`, `styles/campaign-map-responsive.css`, `styles/campaign-map-popups.css`, `styles/campaign-map-selection-inspector.css`, `styles/campaign-map-layer-dock.css`, `styles/campaign-map-scene-inspector.css`, `styles/campaign-map-initiative.css` consume it | Toolbar, popup headers, selection dock, layer/object dock and scene inspector use local sprite icons through `iconSvg` for add, character, creature, object, pan, grid, map image, layers, shapes, drawing, fog, presentation, initiative, music, visibility, duplicate, more and remove actions; do not add external icon packs | Generic `#campaignMapPopup` popups register as modal dialog overlays through `campaignMapPopupController` and now share the migrated map popup frame; `#campaignTokenPopup` registers as a non-modal popover and keeps delayed hover behavior local; the selection dock, layer/object dock and scene inspector are runtime stage surfaces, not persisted overlays | Foundry scenes, Roll20 lighting, Owlbear scene/fog clarity. |
| Knowledge graph and canvas | `.knowledge-graph-document`, `.knowledge-graph-workbench[data-knowledge-graph-migration="phase-7-slice"]`, `.knowledge-graph-canvas-stage`, `.knowledge-graph-canvas-slice-stats`, `.knowledge-graph-canvas-slice-note`, node cards and edges | `styles/knowledge-graph.css`; `js/wiki/knowledgeGraph.js` owns slice counts; `js/wiki/knowledgeGraphPage.js` owns runtime graph rendering | Graph toolbar and node metadata now use local sprite icons for layout/history/fit/refresh/domain hints; no external icon package | Node menu and connect popup register through `popupManager`; the slice note and filterbar counters are runtime canvas controls, not persisted page HTML | Obsidian Graph/Canvas and Miro frames: filters, node/edge tokens, grouping and navigation without becoming a separate whiteboard. |
