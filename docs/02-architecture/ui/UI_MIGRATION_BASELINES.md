...
Status: `0.0.1.8.10` AppShell migration closed and corrected by `0.0.1.8.10.1`-`0.0.1.8.10.4`; `0.0.1.8.11` core content is active and advanced by `0.0.1.8.11.3`.
...
| Card editor and blocks | `.editor-surface`, `.entity-layout`, `.card-shell`, `.editor-page-nav`, `.card-type-trigger`, `.template-block`, `.block-drag-handle`, `.block-drag-preview`, `.block-drop-placeholder`, `.floating-toolbar[role="toolbar"]`, `#blockPopup[data-block-popup-view="type-picker"]` | `styles/editor.css`, `styles/document.css`, `styles/tags.css`, `styles/card-type.css`, `styles/blocks.css`, `styles/popup-block.css`, `styles/popup-block-type.css`, `styles/toolbar.css`, `styles/block-special.css` | Page nav, block controls and Add block options use local sprite icons through `iconSvg`; first-level Add block icons use `document`, `task-tracker`, `grid`, `image` and `settings`; find-in-tree uses `search`, back uses `arrow-left` | `blockPopup`, `linkPopup`, `image-crop-popup` and `toolbarColorPopup` use explicit dialog/popover overlay semantics; the first-level Add block popup now has tokenized type-picker styling; floating text toolbar is moved to the overlay layer for stable viewport positioning | Notion pages/slash commands, Confluence autocomplete, but keep local-first persistent HTML boundary. |
| Properties and sheets | `.card-properties-block`, `.card-properties-grid`, `.card-property-field`, `.character-sheet-block` | `styles/block-properties.css`, `styles/block-character-sheet.css`, `styles/block-character-effects.css`, `styles/ui.css` | Settings uses local `settings`; fields/lock/formula icons are still future sprite candidates | `.property-settings-popup` now uses explicit dialog overlay semantics; visual styling remains feature-owned | Notion databases for field organization, D&D Beyond/Foundry for sheet density and game-readable stats. |
