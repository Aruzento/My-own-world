...
| `0.0.1.8.13` Knowledge graph | Active: `0.0.1.8.13.1` migrated visible-slice clarity, first toolbar/node-card visual language and filterbar counters. Edge states, inspector, overlay visual cleanup, file/CSS split and lifecycle bridge remain open. | `BI-019` is closed; also check `BI-017` and `BI-018` before deeper graph edits. |
...
- `BI-019`: graph hidden-slice clarity is closed in `0.0.1.8.13.1`; future hidden-state work should be a concrete graph bug or an extension of the graph inspector.
