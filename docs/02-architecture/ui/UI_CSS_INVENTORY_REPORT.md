...
Post-`0.0.1.8.13.6` delta: Knowledge Graph has started both CSS and JavaScript ownership splits. `styles/knowledge-graph.css` now owns the document/workbench/canvas/node-card base, while `styles/knowledge-graph-slice.css`, `styles/knowledge-graph-inspector.css` and `styles/knowledge-graph-overlays.css` own slice state, selected-node inspector and node/connect overlays respectively. On the JS side, selected-node inspector helpers, graph node icon mapping and relationship labels/options moved out of `knowledgeGraphPage.js`. The remaining graph risk is canvas renderer/actions/context-menu split and lifecycle bridge work.
...
| File | Lines | UI Risk |
| --- | ---: | --- |
| `js/wiki/knowledgeGraphPage.js` | 6,587 | Too much graph page, canvas UI, actions, overlays and persistence in one file, though `0.0.1.8.13.6` moved selected-node inspector helpers, node icon mapping and relationship labels/options into dedicated JS owners. Remaining `BI-017` risk is canvas renderer/actions/context-menu split before deeper graph behavior. |
...
| `0.0.1.8.13` Knowledge graph | Active: `0.0.1.8.13.1` migrated visible-slice clarity, first toolbar/node-card visual language and filterbar counters; `0.0.1.8.13.2` added selected-node edge states, related/muted node states and the graph inspector dock; `0.0.1.8.13.3` migrated node/connect overlay visuals; `0.0.1.8.13.4` corrected the first layer toward compact chips, meters, icon actions and collapsed relationship editing; `0.0.1.8.13.5` split graph CSS into base, slice, inspector and overlay/menu owners; `0.0.1.8.13.6` split selected-node inspector helpers, graph node icon mapping and relationship labels/options out of the graph page JS. Canvas/actions/context-menu split and lifecycle bridge remain open. | `BI-019` is closed; `BI-017` is partly advanced on CSS and first JS ownership split, but `knowledgeGraphPage.js` still needs renderer/actions/persistence responsibility split; also check `BI-018` before deeper graph edits. |
...
- `BI-017`: Knowledge Graph CSS split is partly advanced in `0.0.1.8.13.5`, and first JS ownership split is partly advanced in `0.0.1.8.13.6`; canvas renderer/actions/context-menu split still belongs to graph redesign or graph lifecycle work.
