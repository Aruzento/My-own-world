...
Related audit tooling: `tools/audit_ui_polish.mjs` is the Phase 9 UI polish audit invoked by `npm run ui:polish:audit` and `npm run verify`; it guards motion, blur and focus regressions across CSS owners. `tests/browser/ui-polish-performance.spec.mjs` is the Phase 9 runtime performance smoke for large migrated UI surfaces.
...
| `0.0.1.8.4` Component catalogue POC | Closed historically as a visible proof of concept; superseded by `0.0.1.8.17`, where the catalogue became dev/test-only instead of normal user-facing Tools UI. | Browser coverage verifies the gated path, focus, Escape close, disabled, invalid, pressed, loading and compact/large density states. |
...
| `0.0.1.8.17` Owner visual completion | Closed at `Usable`: user-facing noise is reduced, the component catalogue is dev/test-only, map popups avoid the Inspector, owner evidence exists at 1440x900 and 1280x720, and future product ideas are moved to `NEXT_PRODUCT_MINI_BACKLOG.md`. | Pause for owner handoff before starting non-design work. Keep `BI-026` before adding visible Knowledge Graph concepts. |
