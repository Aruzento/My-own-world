...
Post-`0.0.1.8.12.4` delta: campaign map CSS now includes `styles/campaign-map-selection-inspector.css` for the selected-object dock and `styles/campaign-map-layer-dock.css` for the layer/object dock. Keep both runtime stage docks in their dedicated owners instead of growing `campaign-map-popups.css` or the stage file with contextual action UI.
...
| `0.0.1.8.12` Campaign map | Active. `0.0.1.8.12.1` migrated the map toolbar foundation, `0.0.1.8.12.2` migrated the shared map popup surface for add/picker, grid, drawing, fog, shapes, layers, initiative and music, `0.0.1.8.12.3` migrated the selected-object inspector dock, and `0.0.1.8.12.4` migrated the layer/object dock. Remaining: map inspector surfaces and advanced contextual actions. | Also check `BI-008`, `BI-009`, `BI-010`, `BI-011` and map performance smoke. |
