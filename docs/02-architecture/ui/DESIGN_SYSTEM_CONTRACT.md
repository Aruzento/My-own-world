...
| `right-panel` | Secondary work panel for an explicit tool or workflow. | Hidden without an active owner. Universal Card Inspector opens it for the currently opened ordinary card; Map and Knowledge Graph inspectors stay inside their workbenches. Its top-bar toggle is user-owned: hide/show remains stable through Inspector rerenders of that card and a different page lifecycle releases the card owner. Do not use it as a decorative context/info panel. |
