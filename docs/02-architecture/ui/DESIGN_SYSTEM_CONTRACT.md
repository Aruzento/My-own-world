...
- current design status: `0.0.1.8.18.6` is `BLOCKED FOR OWNER REVIEW` after an independent read-only visual critic loop. [UI_MIGRATION_BASELINES.md](./UI_MIGRATION_BASELINES.md) remains the comparison manifest, and [UI_OWNER_VISUAL_CRITIC_0.0.1.8.18.md](../../03-testing/UI_OWNER_VISUAL_CRITIC_0.0.1.8.18.md) is the current owner evidence gate. Do not leave the design correction track or start backlog expansion until the owner decides how to proceed.
