...
- Confirmed by `0.0.1.8.12.3` and corrected by `0.0.1.8.12.8`: selected map tokens and shapes now open a runtime-only right-side property Inspector with editable identity/type, position, size, rotation, presentation visibility, style fields and safe local-icon actions. The Inspector separates `Убрать с карты` from destructive card deletion and keeps map business logic unchanged.
- Revised by `0.0.1.8.12.7`: campaign maps should not auto-render the earlier layer/object dock in the default stage. Layer visibility remains reachable through the compact toolbar and existing Layers popup.
- Revised by `0.0.1.8.12.8`: the compact toolbar must stay an icon-only tool strip with small grouped buttons and no oversized text sections. Right-click on a map object opens the same compact object action popup as hover while suppressing the browser context menu.
- Revised by `0.0.1.8.12.9`: campaign maps use a graphic-editor split layout. Canvas tools belong in the left vertical rail; scene/session actions belong in the compact top bar. Future tools should extend sectioned rail groups instead of rebuilding one wide toolbar or adding placeholder panels.
- Revised by `0.0.1.8.12.10`: the split campaign-map toolbar zones must be full-size workbench surfaces with no internal scrollbars. Toolbar labels belong in the body-level floating tooltip layer, not inside clipped button pseudo-elements.
...
Status: started and partially closed by `0.0.1.8.14.1`, with the icon-button regression fixed by `0.0.1.8.14.2`. The board/column/card/checklist UI is migrated to the shared workbench language. Phase 8 has also migrated Settings maintenance, Help/Support guide and World Package manager slices through `0.0.1.8.14.5`; remaining secondary-screen work lives in `0.0.1.8.14` rather than this older phase label.
