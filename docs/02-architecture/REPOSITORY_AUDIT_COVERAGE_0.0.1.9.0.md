...
| `js/taskTracker/taskTrackerDnd.js`, page actions | Task Tracker interaction | DnD and page action writes | REVIEWED - PARTIAL FINDING CLOSED | Main, reviewer 3 | Tracker runtime | Page action writes use page command boundary | High | Task tracker tests | RA-006, RA-028 | Task Tracker write-boundary work is closed by `RCB-006A`; keyboard reorder is closed by `RCB-028`; remaining write-boundary debt stays under split `RCB-006` for Campaign Map writes. |
...
| `tests/browser/task-tracker.spec.mjs` | Browser tests | Task Tracker workflows | REVIEWED - PARTIAL FINDING CLOSED | Main, reviewer 3 | N/A | N/A | High | 26 specs reviewed | RA-006, RA-028 | Pointer and keyboard reorder are covered; remaining direct write-boundary debt stays under split `RCB-006`. |
