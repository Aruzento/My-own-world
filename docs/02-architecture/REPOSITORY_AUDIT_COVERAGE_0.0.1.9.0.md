...
| `js/wiki/knowledgeGraph*.js`, graph overlays/menu modules | Knowledge Graph modules | Graph split owners | REVIEWED - EXISTING FINDING COVERS IT | Main, reviewer 3 | Relationship/page model | Graph command bridge | High | Graph/browser visual tests | RA-010, RA-018 | CSS semantics covered; `knowledgeGraphCanvasHistory.js` now owns canvas history stack/shortcut/control state. |
