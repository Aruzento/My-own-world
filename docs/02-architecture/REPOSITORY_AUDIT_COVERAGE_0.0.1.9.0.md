...
| RA-030 | P3 | CLOSED BY RCB-030 / `0.0.1.10.28` | Focused CSS token/layer guard now covers the command palette, settings/topbar and card type menu drift classes. |
...
| `js/search/*.js`, `js/templates/*.js` | Search/templates | Search read model and page templates | REVIEWED - PARTIAL FINDING CLOSED | Main | PageRepository search | Template storage owns `.my-own-world-templates.json`; page creation uses page command boundary | Medium | Search/template tests | RA-006, RA-007 | `RCB-006B` moved create-from-template page writes behind the existing create-page command; template JSON persistence remains an allowed template-storage responsibility. |
...
| `js/editor/blocks/blockRuntimeControls.js`, `js/ui/itemSets.js` list normalization | Runtime block controls | Runtime controls, short description, list kind normalization | REVIEWED - PARTIAL FINDING CLOSED | Main | Runtime UI and item picker creation | Item pages use create-page command; set chips save through editor | Medium | Item set/property browser tests | RA-006 | `RCB-006C` moved item picker page creation behind the existing create-page command. Runtime normalization is acceptable temporary debt. |
...
| `js/editor/campaignMap.js`, runtime/render adapter files | Campaign Map runtime | Map stage and runtime orchestrator | REVIEWED - FINDING CLOSED | Main, reviewer 2 | Map page content/runtime store | Map special save | High | Campaign map browser tests | RA-006, RA-011 | `RCB-006D` closed feature-level page write bypasses; ordinary map save remains the Campaign Map special-save owner. |
| `js/editor/campaignMapDataSerializer.js`, model/store files | Campaign Map model/store | Map durable model and serializer | REVIEWED - FINDING CLOSED | Main | Map page data | Serializer only; save owner writes | High | Campaign map data tests | RA-006 | Serializer/model paths remain internal; direct feature write boundary closed by `RCB-006D`. |
| `js/editor/campaignMapToolbar*.js`, popup controller files | Campaign Map toolbar/popups | Map controls and overlays | REVIEWED - FINDING CLOSED | Main | Runtime UI | Save via map flows | Medium | Popup/map browser tests | RA-011 | `RCB-011` moved exact popup/template escaping duplicates to `campaignMapPopupMarkup.js`; shared popup positioning ownership remains from 8.18.5. |
| `js/editor/campaignMapToken*.js` | Campaign Map tokens | Token actions/popups/drag | REVIEWED - FINDING CLOSED | Main, reviewer 2 | Token runtime/map data and linked card pages | Map save/page command helpers | High | Map UI tests | RA-006, RA-011 | `RCB-006D` routes linked-card HP updates and duplicate-token page normalization through `PageCommandService`; no product feature added. |
...
| `js/editor/campaignMapMusic/Initiative/Presentation*.js` | Map session features | Music, initiative, presentation sync | REVIEWED - FINDING REDUCED | Main | Map/session data | Map save/presentation sync | Medium | Map initiative/presentation tests | RA-011 | `RCB-011` centralized exact music/initiative popup escaping duplicates and initiative `rollD20()` ownership. Presentation-specific helpers were not merged by this exact-duplicate leaf. |
| `js/properties/*.js` | Properties model | Properties data/model/calculation/layout decisions | REVIEWED - NO MATERIAL FINDING | Main | Properties model | Editor save | Medium | Property tests | none | `RCB-009` moved pure layout overlap/collision decisions into `propertyLayoutModel`. |
| `js/editor/propertiesSettingsPopup.js` | Properties popup | Layout, drag, popup orchestration | REVIEWED - PARTIAL FINDING REDUCED | Main | Runtime layout | Writes properties into page DOM | High | Property browser tests | RA-009 | Popup remains UI/orchestration owner; pure collision decisions moved to `propertyLayoutModel`. |
...
| `js/editor/characterSheetBlock.js`, properties calculations | Character sheet | Character UI/calculations | REVIEWED - PARTIAL FINDING CLOSED | Main | Character/Properties | Editor save | Medium | Character/property tests | RA-020 | Character Sheet token ownership is closed by `RCB-020`; the renderer behavior was unchanged. |
...
| `js/taskTracker/taskTrackerDnd.js`, page actions | Task Tracker interaction | DnD and page action writes | REVIEWED - FINDING CLOSED | Main, reviewer 3 | Tracker runtime | Page action writes use page command boundary | High | Task tracker tests | RA-006, RA-028 | Task Tracker write-boundary work is closed by `RCB-006A`; keyboard reorder is closed by `RCB-028`; full `RCB-006` is closed by `RCB-006D`. |
| `js/tree/tree*.js` render/keyboard | Tree | Tree rendering and keyboard contract | REVIEWED - FINDINGS CLOSED | Main, reviewer 3 | Runtime tree over pages; page resolution through PageRepository | Opens/current page | High | Tree accessibility and DnD tests | RA-007, RA-022, RA-028 | Action menu keyboard gap is closed by `RCB-022`; keyboard reorder is closed by `RCB-028`; tree-adjacent lookup/query reads are closed by `RCB-007D` while render snapshots remain intentional. |
| `js/tree/treeDragDrop.js`, context/order files | Tree DnD/context | Tree move/reorder/context menu | REVIEWED - FINDINGS CLOSED | Main, reviewer 3 | Page parent/order; DnD target/context lookup through PageRepository | Tree position writes | High | Tree DnD tests | RA-002, RA-007, RA-028 | Durable batch rollback is closed by `RCB-002`; keyboard reorder now reuses the same move/write owner; tree-adjacent lookup/query reads are closed by `RCB-007D`. |
...
| `js/ui/worldPackageManager.js` | World Package UI | Import/export UI manager | REVIEWED - PARTIAL FINDING CLOSED | Main, reviewer 2 | Package preview/export reads through PageRepository; runtime modal UI | Import/export services | High | World package tests | RA-007, RA-019 | `RCB-007C` moved export-all, branch traversal and preview conflict reads to `PageRepository`; God-file note not escalated. |
...
| `styles/knowledge-graph*.css` | Knowledge Graph CSS | Graph canvas/inspector/overlay styles | REVIEWED - PARTIAL FINDING CLOSED | Main | N/A | N/A | Medium | Graph visual tests | RA-010 | `RCB-010` removed the recorded graph `font-size: 0` scale hack and old hidden menu/toolbar label CSS; broader token debt remains under RA-020/RA-030. |
...
| `styles/app-topbar.css`, settings CSS | Settings/topbar CSS | Settings and top shell styles | REVIEWED - FINDINGS CLOSED | Main, reviewer 3 | N/A | N/A | Medium | UI polish audit | RA-020, RA-030 | Settings `--mow-line-height-normal` and `--mow-input-color` are closed by `RCB-020`; focused token/layer drift guard coverage is closed by `RCB-030`. |
...
| `tools/run_browser_smoke.mjs`, desktop gate/clickthrough tools | Browser/desktop gates | Smoke and release gates | REVIEWED - FINDINGS CLOSED | Main | N/A | Writes reports | High | Desktop/browser smoke | RA-003, RA-004, RA-014 | RA-003 and RA-004 are closed; RCB-012 now documents the visual evidence-smoke policy without adding strict pixel baselines. |
...
| `tests/browser/*app-shell*`, tree specs | Browser tests | Shell/tree/accessibility/DnD | REVIEWED - FINDINGS CLOSED | Main, reviewer 3 | N/A | N/A | High | 26 specs reviewed | RA-022, RA-028 | RA-022 keyboard action gap and RA-028 keyboard reorder are both covered. |
...
| `tests/browser/task-tracker.spec.mjs` | Browser tests | Task Tracker workflows | REVIEWED - FINDING CLOSED | Main, reviewer 3 | N/A | N/A | High | 26 specs reviewed | RA-006, RA-028 | Pointer and keyboard reorder are covered; direct write-boundary debt from the original `RCB-006` scope is closed. |
| `tests/browser/knowledge-graph`, popup, property, tables specs | Browser tests | Graph/popup/properties/tables | REVIEWED - FINDINGS CLOSED | Main | N/A | N/A | High | 26 specs reviewed | RA-017, RA-018, RA-019 | RA-017 is closed by `RCB-017`; RA-018 is closed by `RCB-018`; RA-019 is closed by `RCB-019`. |
| `tests/browser/visual-regression.spec.mjs`, security/perf/assets specs | Browser tests | Visual evidence and high-risk smoke | REVIEWED - FINDING CLOSED | Main, reviewer 3 | N/A | N/A | Medium | 26 specs reviewed | RA-014 | `RCB-012` documents the suite as evidence smoke, not strict pixel baseline, and adds a static policy guard. |
...
| Tree | page `parent`/`order` metadata | Tree render over `state.pages` | pageStorage tree position writes | Tree render snapshot plus PageRepository for lookup/query reads | PageRecord metadata | pageStorage tree move rollback | Batch durable rollback closed by `RCB-002`; direct lookup/query bypass closed by `RCB-007D` | RA-002, RA-007 |
...
| Campaign Map | Map page content/data HTML | CampaignMap model/store/DOM | Map special save and helpers | Map serializers/model | campaignMapDataSerializer | Page command for special save and helper page writes | Feature helper bypasses closed by `RCB-006D` | RA-006, RA-011 |
| Tokens | Map page token data and linked card pages | Map token DOM/store | Map token actions/save | Map store/Character bridge | Map serializer | Map save/page command helpers | Linked-card/duplicate-page direct writes closed by `RCB-006D` | RA-006 |
| Initiative | Map initiative data | Initiative model/popup runtime | Initiative popup/model save through map | Initiative model | Map/initiative serializer | Map save | Exact helper duplication closed by `RCB-011` | RA-011 |
...
| Knowledge Graph | Page relationships metadata/content | Graph model/canvas state | Graph command bridge/page writes | Graph modules/PageRepository | Graph relationship/page serializers | Graph command bridge/history | Page direct reads closed by `RCB-007B`; coordinator debt remains RA-008 | RA-007, RA-008, RA-018 |
...
| World Packages | package JSON/assets | WorldPackage model/manager preview | Import/export services | PageRepository for manager preview/export page reads; WorldPackage model/import service for package data | WorldPackage model/import service | Import service rollback | UI manager size/modality; tree-adjacent reads remain outside this domain | RA-007, RA-019 |
...
| Task tracker save | special save/page action | Special save/page command actions | `writePageContent` via page command | write revision for page actions | PageCommandService rollback for page actions | command notification | closed by `RCB-006A` | RA-006 |
| Campaign Map save | special save/map actions | Map special save/page command helpers | `writePageContent` via page command or allowed map save owner | write revision for helper page writes | PageCommandService rollback for helper page writes | command notification | closed by `RCB-006D` | RA-006 |
| Graph relation mutation | graph command bridge/actions | Graph command bridge/page command adjacent | `writePageContent` | command bridge checks | graph restore/history | `notifyPageUpdated` | graph coordinator page reads closed by `RCB-007B` | RA-007, RA-008 |
...
| Subsystem | Result |
| --- | --- |
| Campaign Map | Aggregation did not hide a new P1. Direct write/helper debt is closed by `RCB-006D`; exact helper duplication is closed by `RCB-011`. |
| Task Tracker | Structural icon-only cleanup remains good; direct page actions are closed by `RCB-006A`; keyboard reorder is closed by `RCB-028`. |
