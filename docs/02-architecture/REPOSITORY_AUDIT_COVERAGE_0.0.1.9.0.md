...
| `js/tree/tree*.js` render/keyboard | Tree | Tree rendering and keyboard contract | REVIEWED - FINDINGS CLOSED | Main, reviewer 3 | Runtime tree over pages | Opens/current page | High | Tree accessibility and DnD tests | RA-022, RA-028 | Action menu keyboard gap is closed by `RCB-022`; keyboard reorder is closed by `RCB-028`. |
| `js/tree/treeDragDrop.js`, context/order files | Tree DnD/context | Tree move/reorder/context menu | REVIEWED - FINDINGS CLOSED | Main, reviewer 3 | Page parent/order | Tree position writes | High | Tree DnD tests | RA-002, RA-028 | Durable batch rollback is closed by `RCB-002`; keyboard reorder now reuses the same move/write owner. |
