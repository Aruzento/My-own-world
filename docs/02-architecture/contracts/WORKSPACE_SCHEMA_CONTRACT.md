...
- diagnostics UI показывает grouped recovery report: группы проблем, количество ошибок, legacy warnings, warnings requiring review и safe-after-backup actions;
- persisted UI repair сейчас есть только для `detach-page-parent-to-root`: он создает backup gate, затем через page command path очищает missing parent и повторяет diagnostics;
- тесты проверяют invalid JSON, missing id, broken parent, duplicated title, broken map token, broken task data, template errors, asset reference errors, grouped recovery report, malformed/partial fallback cases, missing asset paths, backup-before-repair failure и schema upgrade gate;
- browser smoke проверяет recovery fallback screen, grouped malformed workspace report and diagnostics backup gate failure.

Следующий этап:

- расширить persisted recovery actions beyond page-parent cleanup: map token cleanup, task reference cleanup, link cleanup and asset repair preview;
- подключить schema validation к рискованным операциям: import, schema upgrade, bulk operations;
- добавить browser/storage tests для restore preview, partial restore, asset repair, link cleanup и будущих persisted repair-actions.
...
- UI показывает grouped report и может применить persisted `detach-page-parent-to-root` после backup. Остальные safe model-level actions остаются preview, пока для них нет надежной записи через persistent command path, storage/browser tests и повторной validation.
