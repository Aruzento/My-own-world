...
| World Package manager (`#worldPackagePopup`) | `js/ui/worldPackageManager.js` | Static in `index.html` | Yes | Static/fixed CSS; opened through controller `open()` | Shared | Shared | Modal focus trap and focus return to Tools trigger | `dialog`, modal | One native `window.confirm` for deleting a package file | Yes, for delete confirmation only | Main manager dialog is canonical; the package-file delete confirmation still bypasses shared app popup UI. | B |
...
| Confirm popup | `js/ui/confirmPopup.js` | Dynamic body node | Yes | `openPopupNearAnchor` / `popupPosition` | Shared | Shared | Non-modal anchored confirm | `popover` | None material | No | Existing shared confirm primitive is PopupManager-backed. | A |
