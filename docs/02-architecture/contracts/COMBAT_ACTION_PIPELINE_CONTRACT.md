...
Every outcome captures the exact target PageRecord base before RNG and validates that the runtime target content matches the durable target content through `evaluatePageWritePrecondition`. The same captured base is revalidated before a page write or EventStore append. Miss, zero and clamped outcomes therefore have no page write but still cannot produce audit history from unsaved or stale Character/AC data. A hit additionally requires `CharacterHealthMutationPlan.expectedBase` to equal this captured base.
