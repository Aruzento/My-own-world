...
Cleanup status: closed by `RCB-001B` in `0.0.1.10.3`. `PageIndex` now removes old keys from the last indexed metadata shape before storing the new live page shape, and `TreeIndex` uses its indexed parent bucket when the live page was already mutated. The regressions prove title, alias, tag and type updates, including empty-value transitions, without a full index rebuild.
...
Cleanup status: closed by `RCB-003` in `0.0.1.10.5`. The native click-through runner now uses one shared status helper for exit code and markdown status. Unexpected `pageerror` and `console.error` fail the smoke; `console.warning` stays visible diagnostic evidence; the only allowlisted runtime event is the exact Chromium WebView ResizeObserver notification documented in the runner.
...
Cleanup status: closed by split leaves `RCB-007A` in `0.0.1.10.14`, `RCB-007B` in `0.0.1.10.35`, `RCB-007C` in `0.0.1.10.36` and `RCB-007D` in `0.0.1.10.37`. Final classification:
...
- `MIGRATED IN RCB-007D`: tree-adjacent page resolution reads now use `PageRepository` where they are lookup/query behavior: `treeUtils.canMovePage()` receives a repository-backed page snapshot, DnD target/current-page refresh uses `getPageById()`, context delete branch/fallback reads use `getChildren()`, `getPageById()` and `getAllPages()`, tree keyboard/reveal helpers use `getPageById()` / `getParentChain()`, and editor back navigation resolves pages through `getPageById()`.
...
- `TREE RENDER / TRANSACTION SNAPSHOT - ALLOWED`: `renderTree()` still projects `state.pages` into the visible tree, and pointer/keyboard tree move planning still passes `state.pages` as one consistent mutable transaction snapshot into `createTreeMovePlan()`. These are not feature lookup/query bypasses.
- `RUNTIME STORE ACCESS - ALLOWED`: `state.currentPage` reads in item set/tree/navigation code remain runtime selection access, not page collection lookup.
- `RCB-007 CLOSURE`: no inappropriate page lookup/query bypass remains in the original RCB-007 scope after the final search.
...
| RA-003 | CLOSED BY RCB-003 / `0.0.1.10.5` | `tools/run_desktop_native_clickthrough.mjs`, `tests/desktopNativeSmokeStatus.test.mjs` | Native click-through final status now fails on unexpected page/console runtime errors and reports allowlisted runtime events separately. |
