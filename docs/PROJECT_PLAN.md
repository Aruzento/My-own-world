...
4. **Visual Regression / UX Safety** - многие реальные баги были визуальными.
5. **Popup Lifecycle Standardization** - popup-система разрослась и требует единого жизненного цикла.
6. **Properties Model / Character Calculations** - первый крупный продуктовый слой после стабилизации данных.
...
1.7. Добавить validation gate перед будущими schema upgrades: **не сделано**.

1.8. Добавить browser/storage tests для recovery fallback, templates/assets validation и будущих repair-actions: **не сделано**.
...
2.5. Добавить tests на восстановление карты, карточки и таск-трекера: **частично сделано**.
Покрыты manifest/id и базовый сервис. Нужны интеграционные browser/storage tests.

2.6. Добавить backup assets по `AssetReference`: **не сделано**.

2.7. Добавить политику очистки старых backup: **не сделано**.

### 3. Campaign Map Performance Gate

Статус: **частично сделано, требуется усиление**.
Приоритет: **P1**.
...
3.4. Ввести performance budgets: **сделано базово**.
Нужно сделать budgets более обязательными для CI и stress smoke.

3.5. Оптимизировать presentation full-sync: **частично сделано**.
Убрано лишнее чтение модели из DOM во время sync. Нужны batch updates, diff by id и запрет full repaint без причины.

3.6. Добавить performance regression smoke: **сделано базово**.
Нужно расширить сценарии на fog paint, большую карту, много токенов/фигур и presentation.

3.7. Оптимизировать туман: throttling, dirty regions, меньше sync при рисовании: **не сделано полностью**.
Были точечные улучшения, но лаги при тумане остаются риском.

3.8. Добавить diagnostics UI только для dev/debug режима: **не сделано**.
...
5.5. Проверить popup карты, инициативы, тумана, слоев, шаблонов и backup UI.
...
Следующий рабочий пункт: **1.2-1.5** или **2.5-2.7**, если продолжаем закрывать P0 data safety.

После закрытия P0 data safety логичный переход: **3. Campaign Map Performance Gate**, затем **4. Visual Regression / UX Safety**.
