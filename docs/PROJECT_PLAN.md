...
Убрано лишнее чтение модели из DOM во время sync, live-sync уже идет item-level по id, performance smoke проверяет bounded full-sync и item sync. В план остаются batch updates, diff by id для большего числа операций и запрет full repaint без причины.
...
3.7. Оптимизировать туман: throttling, dirty regions, меньше sync при рисовании: **частично сделано**.
Туман получил dirty-region counters для diagnostics, live presentation sync остается throttled, а locked fog zone drag больше не пересобирает все runtime-зоны и не коммитит DOM на каждом pointermove. В план остаются настоящие dirty-region save/sync и stress-тест с реальным pointer painting.
