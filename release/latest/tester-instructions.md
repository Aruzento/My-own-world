...
- Current Data Safety scope is closed for restore preview, partial restore, grouped asset/link/orphan diagnostics and backup-gated selected repair. NF-001 edit-session conflict protection is closed for the current PageRecord-backed scope. NF-002 Safe Dice Engine is closed: parser, evaluator, explicit limits, deterministic RNG, structured RollResult, advantage/disadvantage, natural d20 critical classification, universal public API and initiative parity now exist. NF-003 Event / Roll / Combat Log + Transactions is active as a foundation phase; `0.0.1.15.1` defined the owner map and event/transaction contract, `0.0.1.15.2` added a pure runtime transaction model, `0.0.1.15.3` added the durable append/read/reload event sidecar, `0.0.1.15.4` added strict v1 event type payload contracts, `0.0.1.15.5` added the first durable roll-event consumer, `0.0.1.15.6` added the first state-changing resource event transaction, and `0.0.1.15.7` added first reversible compensation for that stateful transaction. Event UI, dice UI, broad undo/redo and combat behavior are still not implemented.
