...
For the campaign map toolbar, popup, selection-inspector, layer/object dock, scene-state inspector and group contextual actions redesign slice, use:

```powershell
npm run test:browser -- --grep "campaign-map-toolbar-uses-migrated-mode-action-groups|campaign-map-popups-use-migrated-shared-frame|campaign-map-selection-inspector|campaign-map-layer-dock|campaign-map-scene-inspector|visual-safety-captures-core-surfaces"
npm run test:browser -- tests/browser/campaign-map-ui.spec.mjs
```

For the Knowledge Graph visible-slice, selected-node inspector and edge-state slice, use:
...
17. In the Knowledge Graph, drag a node, undo/redo, right-click a node and check relationship actions. The context menu should open near the clicked node and stay inside the viewport.
18. In the Knowledge Graph, create a connection through the connect popup; node/connect overlays should stay within the viewport and close cleanly by Escape/outside click.
...
- Knowledge Graph is currently a useful visual workbench with visible-slice clarity and selected-node relationship clarity, not the final full relationship-management/lifecycle-hardened surface.
