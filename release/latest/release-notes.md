...
- UI redesign Phase 6 campaign map has started with `0.0.1.8.12.1` and `0.0.1.8.12.2`: the map toolbar is now grouped into creation, scene/view, tools and live-session actions, and the add/picker, grid, drawing, fog, shapes, layers, initiative and music popups now share one compact map overlay frame. Both slices use local sprite icons, shared control tokens, accessibility metadata and preserved legacy selectors so behavior stays stable while the UI becomes easier to read.
...
- Full browser smoke passed inside `npm run desktop:gate` with 105 browser tests after the AppShell rail/profile, Explorer action, block DnD, Add block popup, card editor toolbar, card-block select, template picker, command-palette/deep-search and campaign-map toolbar/popup corrections.
...
- `npm run test:browser -- tests/browser/campaign-map-ui.spec.mjs` passed locally with 11 browser tests, including the `0.0.1.8.12.1` toolbar grouping/accessibility/overflow guard, the new `0.0.1.8.12.2` shared popup-frame guard, plus existing map token, drawing, music, layers, fog and selection flows.
- `npm run test:browser -- tests/browser/campaign-map-data.spec.mjs`, `campaign-map-performance.spec.mjs`, `campaign-map-presentation.spec.mjs` and `campaign-map-initiative.spec.mjs` passed locally after the toolbar and popup migrations.
...
- `npm run desktop:gate` passed locally, including docs index, skills validation, verify, 105 browser smoke tests, desktop frontend prepare, packaging smoke, desktop environment check and Tauri cargo check. The large-workspace desktop smoke was skipped because no external workspace path was provided.
