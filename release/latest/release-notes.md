...
- The `0.0.1.8` redesign pass is accepted by the owner for the current stage. Final visual polish remains future debt; the active project phase is `0.0.1.11.0` Existing P1 Stabilization in `docs/01-delivery/PROJECT_PLAN.md`. `0.0.1.11.5` `BUG-003` Desktop Real-App Verification is `VERIFIED / CLOSED`; next leaf is `0.0.1.11.6` Map Presentation.
