...
- Restore preview, partial restore, grouped asset/link/orphan diagnostics, backup-gated selected repair, edit-session conflict protection and NF-002 Dice Engine are implemented for the current safety scope. The active roadmap risk is the NF-003 event/transaction foundation: `0.0.1.15.3` added the owner-approved durable `.my-own-world-events/transactions.v1.jsonl` store, `0.0.1.15.4` added strict v1 event payload contracts, `0.0.1.15.5` added the first durable roll-event consumer, and `0.0.1.15.6` added the first state-changing resource event transaction. Event UI, backup/restore inclusion policy, undo and combat behavior are still not implemented.

### Verification Snapshot

- Latest standalone `npm run test` passed locally with 525 node/static tests.
- Latest `npm run verify` passed locally with encoding/syntax checks, 525 tests, synthetic large-workspace smoke, `git diff --check` and docx zip validation.
