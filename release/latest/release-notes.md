...
- UI redesign Phase 8 secondary screens has started with `0.0.1.8.14.1`: task tracker now uses a compact workbench toolbar, local sprite icon actions, quiet column/task/checklist counters, checklist progress bars and tokenized board/column/card surfaces without changing task tracker data persistence.
...
- Full browser smoke passed inside `npm run desktop:gate` with 113 browser tests after the AppShell rail/profile, Explorer action, block DnD, Add block popup, card editor toolbar, card-block select, template picker, command-palette/deep-search, campaign-map toolbar/popup/selection/group-action, Knowledge Graph `0.0.1.8.13.1`-`0.0.1.8.13.11` corrections and task tracker `0.0.1.8.14.1` workbench UI guard.
...
- `npm run test:browser -- tests/browser/task-tracker.spec.mjs` passed locally with 3 focused browser tests covering task tracker persistence, legacy JSON upgrade and the `0.0.1.8.14.1` compact workbench UI surface.
...
- The first full `desktop:gate` attempt had one transient failure in the unrelated `property-settings-gear-opens-soft-settings-popup` browser test; the focused test and full `tests/browser/property-blocks.spec.mjs` reruns passed before the successful second desktop gate.
...
- `npm run desktop:gate` passed locally, including docs index, skills validation, verify, 113 browser smoke tests, desktop frontend prepare, packaging smoke, desktop environment check and Tauri cargo check. The large-workspace desktop smoke was skipped because no external workspace path was provided.
