...
- UI redesign Phase 7 Knowledge Graph now includes `0.0.1.8.13.1`-`0.0.1.8.13.9`: the graph canvas keeps hidden-slice clarity but presents it more visually, with compact status chips, a slice meter, icon-first toolbar/filter/node actions, short selected-node relation chips, and relationship editing collapsed behind `Связи`. Graph CSS is now split into base, slice, inspector and overlay/menu owner files, and the JS ownership split now moves selected-node inspector helpers, graph node icon mapping, relationship labels/options, canvas controls/filterbar/slice-meter helpers, canvas node/edge renderer helpers and runtime filter/layout/slice/zoom actions out of the page monolith.
...
- Full browser smoke passed inside `npm run desktop:gate` with 112 browser tests after the AppShell rail/profile, Explorer action, block DnD, Add block popup, card editor toolbar, card-block select, template picker, command-palette/deep-search, campaign-map toolbar/popup/selection/group-action and Knowledge Graph `0.0.1.8.13.1`-`0.0.1.8.13.9` corrections, including laconic graph UI, CSS ownership split and graph inspector/labels/canvas-controls/canvas-renderer/canvas-actions JS split.
