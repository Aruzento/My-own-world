...
- UI redesign Phase 6 campaign map has advanced through `0.0.1.8.12.3`: the map toolbar is grouped into creation, scene/view, tools and live-session actions; add/picker, grid, drawing, fog, shapes, layers, initiative and music popups share one compact map overlay frame; and selecting a token or shape now opens a compact runtime dock with identity, position, visibility, key token stats and safe contextual actions. These slices use local sprite icons, shared control tokens, accessibility metadata and preserved legacy selectors so behavior stays stable while the UI becomes easier to read.
...
- Full browser smoke passed inside `npm run desktop:gate` with 106 browser tests after the AppShell rail/profile, Explorer action, block DnD, Add block popup, card editor toolbar, card-block select, template picker, command-palette/deep-search and campaign-map toolbar/popup/selection-dock corrections.
...
- `npm run test:browser -- tests/browser/campaign-map-ui.spec.mjs` passed locally with 12 browser tests, including the `0.0.1.8.12.1` toolbar grouping/accessibility/overflow guard, the `0.0.1.8.12.2` shared popup-frame guard, the new `0.0.1.8.12.3` selection inspector/safe-action guard, plus existing map token, drawing, music, layers, fog and selection flows.
...
- `npm run desktop:gate` passed locally, including docs index, skills validation, verify, 106 browser smoke tests, desktop frontend prepare, packaging smoke, desktop environment check and Tauri cargo check. The large-workspace desktop smoke was skipped because no external workspace path was provided.
