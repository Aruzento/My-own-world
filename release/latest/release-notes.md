...
- Restore preview, partial restore, grouped asset/link/orphan diagnostics, backup-gated selected repair, edit-session conflict protection and NF-002 Dice Engine are implemented for the current safety scope. The active roadmap risk is the NF-003 event/transaction foundation: `0.0.1.15.3` added the owner-approved durable `.my-own-world-events/transactions.v1.jsonl` store, `0.0.1.15.4` added strict v1 event payload contracts, `0.0.1.15.5` added the first durable roll-event consumer, `0.0.1.15.6` added the first state-changing resource event transaction, `0.0.1.15.7` added first reversible compensation for that stateful transaction, and `0.0.1.15.8` added a bounded public read/query facade. Event UI, backup/restore inclusion policy, broad undo/redo and combat behavior are still not implemented.
