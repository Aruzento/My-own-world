...
- UI redesign Phase 7 Knowledge Graph now includes `0.0.1.8.13.1`-`0.0.1.8.13.3`: the graph canvas shows visible/total/hidden counters, status says `показано X из Y`, hidden reasons separate filters, standard slice and canvas limit, the slice notice offers `Все связи` / `Уточнить поиск`, toolbar/node cards use local sprite icons plus tokenized domain markers, selecting a node highlights visible relationships and opens a compact workbench inspector, and node/connect overlays now use dark editor-grade grouped controls with compact editable relationship rows.
...
- Knowledge Graph is usable as a canvas workbench and now explains hidden canvas slices plus selected-node relationships with cleaned-up node/connect overlays, but file split and lifecycle cleanup remain Phase 7/backlog work.
