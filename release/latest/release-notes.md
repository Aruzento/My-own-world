...
- Full browser smoke passed inside `npm run desktop:gate` with 112 browser tests after the AppShell rail/profile, Explorer action, block DnD, Add block popup, card editor toolbar, card-block select, template picker, command-palette/deep-search, campaign-map toolbar/popup/selection/group-action and Knowledge Graph `0.0.1.8.13.1`-`0.0.1.8.13.11` corrections, including laconic graph UI, CSS ownership split, graph JS owner splits and command-lifecycle relationship persistence.
