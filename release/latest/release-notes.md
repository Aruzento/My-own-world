...
- UI redesign Phase 8 secondary screens has continued through `0.0.1.8.14.6`: task tracker now uses a compact workbench toolbar, local sprite icon actions, quiet counters and guarded nested-icon button clicks; the Settings popup now presents appearance, backup, asset health and workspace diagnostics as one coherent maintenance panel; Tools help routes now open a Help/Support/Release guide; and Tools now includes a real `Пакеты мира` manager for branch/world export, package library, JSON import preview, backup-gated page/rulePackage import, non-destructive conflict modes (`Стоп`, `Только новые`, `Копии`) and asset preflight.
...
- `npm run verify` passed locally with 277 node tests and large-workspace smoke; the same verify step also passed inside `npm run desktop:gate`.
