...
- UI redesign Phase 9 closure sync now has `0.0.1.8.15.5`: plan, dashboard, work log, UI architecture docs, release notes, tester instructions, project file audit and generated manual are synchronized after the polish/performance/visual/dead-CSS pass. The next design-system work is `0.0.1.8.16` broader visual regression coverage.
...
- Full browser smoke passed inside `npm run desktop:gate` with 115 browser tests after the AppShell rail/profile, Explorer action, block DnD, Add block popup, card editor toolbar, card-block select, template picker, command-palette/deep-search, campaign-map toolbar/popup/selection/group-action, Knowledge Graph `0.0.1.8.13.1`-`0.0.1.8.13.11` corrections and Phase 8 task tracker / Settings maintenance / Help-Support / World Package manager guards.
...
- `npm run test:browser -- tests/browser/visual-regression.spec.mjs` passed locally with 4 focused browser tests, including the Phase 0 baseline attachments and the new Phase 9 theme-scale workbench attachments.
...
- `npm run desktop:gate` passed locally, including docs index, skills validation, verify, 115 browser smoke tests, desktop frontend prepare, packaging smoke, desktop environment check and Tauri cargo check. The large-workspace desktop smoke was skipped because no external workspace path was provided.
