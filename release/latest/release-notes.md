...
- UI redesign Phase 7 Knowledge Graph now includes `0.0.1.8.13.1`-`0.0.1.8.13.5`: the graph canvas keeps hidden-slice clarity but presents it more visually, with compact status chips, a slice meter, icon-first toolbar/filter/node actions, short selected-node relation chips, and relationship editing collapsed behind `Связи`. Graph CSS is now split into base, slice, inspector and overlay/menu owner files so future graph work has a cleaner foundation.
...
- Knowledge Graph is usable as a canvas workbench and now handles hidden canvas slices plus selected-node relationships with a more laconic visual first layer and a first CSS ownership split, but JavaScript split and lifecycle cleanup remain Phase 7/backlog work.
