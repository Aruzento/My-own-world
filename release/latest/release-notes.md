...
- UI redesign Phase 8 secondary screens has continued through `0.0.1.8.14.4`: task tracker now uses a compact workbench toolbar, local sprite icon actions, quiet counters and guarded nested-icon button clicks; the Settings popup now presents appearance, backup, asset health and workspace diagnostics as one coherent maintenance panel; Tools help routes now open a Help/Support/Release guide; and Tools now includes a real `Пакеты мира` manager MVP for branch/world export, package library, JSON import preview and backup-gated page-only import with conflict/assets/rulePackage blockers.
...
- `npm run verify` passed locally with 275 node tests and large-workspace smoke; the same verify step also passed inside `npm run desktop:gate`.
...
- `npm run test:browser -- tests/browser/world-package.spec.mjs` passed locally for the `0.0.1.8.14.4` World Package manager MVP, including Tools entry, branch export, library refresh, conflict preview blocking, page-only import after backup, sanitized imported HTML and layout guards.
