...
- UI redesign Phase 0 now has `UI_MIGRATION_BASELINES.md` and expanded visual smoke attachments for shell, Settings maintenance, tree, editor, Properties, map, graph, task tracker and shared popover surfaces before broader migration.
...
- UI redesign Phase 8 secondary screens has continued through `0.0.1.8.14.2`: task tracker now uses a compact workbench toolbar, local sprite icon actions, quiet counters and guarded nested-icon button clicks; the Settings popup now presents appearance, backup, asset health and workspace diagnostics as one coherent maintenance panel with local sprite actions and explicit health/backup/restore/asset/danger markers.
...
- Full browser smoke passed inside `npm run desktop:gate` with 114 browser tests after the AppShell rail/profile, Explorer action, block DnD, Add block popup, card editor toolbar, card-block select, template picker, command-palette/deep-search, campaign-map toolbar/popup/selection/group-action, Knowledge Graph `0.0.1.8.13.1`-`0.0.1.8.13.11` corrections and Phase 8 task tracker / Settings maintenance guards.
