...
- The `0.0.1.8` redesign pass is accepted by the owner for the current stage. Final visual polish remains future debt; the active project phase is `0.0.1.11.0` Existing P1 Stabilization in `docs/01-delivery/PROJECT_PLAN.md`. `0.0.1.11.2` `BI-010` Campaign Map Toolbar is not reproduced with strong current evidence; next leaf is `0.0.1.11.3` `BI-011` Creature Skills Encoding.
- Restore preview, partial restore, link cleanup and asset repair are still future data-safety work.

### Verification Snapshot

- `npm run verify` passed locally with 285 node tests and large-workspace smoke; the same verify step also passed inside `npm run desktop:gate`.
...
- `npm run test:browser -- tests/browser/campaign-map-ui.spec.mjs` passed locally with 15 browser tests, including the `0.0.1.8.12.10` split toolbar/title/tool-rail guards for full-size zones, no internal scrollbars, unclipped floating tooltip labels and absence of duplicate scene/layer panels, property Inspector and custom right-click guards, the `0.0.1.8.12.2` shared popup-frame guard, the `0.0.1.8.12.6` group visibility action guard, plus existing map token, drawing, music, layers, fog and selection flows.
