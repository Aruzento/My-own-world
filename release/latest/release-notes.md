...
- The `0.0.1.8` redesign pass is accepted by the owner for the current stage. Final visual polish remains future debt; the active project phase is `0.0.1.11.0` Existing P1 Stabilization in `docs/01-delivery/PROJECT_PLAN.md`. `0.0.1.11.3` `BI-011` Creature Skills Encoding is fixed and `BI-022` is done; next leaf is `0.0.1.11.4` Large Workspace UX.
