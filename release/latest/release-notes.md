...
- UI redesign Phase 6 campaign map is now closed at `Usable` through `0.0.1.8.12.6`: the map toolbar is grouped into creation, scene/view, tools and live-session actions; add/picker, grid, drawing, fog, shapes, layers, initiative and music popups share one compact map overlay frame; selecting a token or shape opens a compact runtime dock with identity, position, visibility, key token stats and safe contextual actions; multi-selected tokens/shapes show visible/hidden counters and can be hidden from or shown to players as one group; the map stage has a compact layer/object dock with token/object/shape counters, hidden-object count, per-layer summaries and quick visibility toggles; and the scene-state inspector shows map background, grid and fog readiness with direct actions into the existing map-image, Grid and Fog flows. These slices use local sprite icons, shared control tokens, accessibility metadata and preserved legacy selectors so behavior stays stable while the UI becomes easier to read.
- UI redesign Phase 7 Knowledge Graph now includes `0.0.1.8.13.1` and `0.0.1.8.13.2`: the graph canvas shows visible/total/hidden counters, status says `показано X из Y`, hidden reasons separate filters, standard slice and canvas limit, the slice notice offers `Все связи` / `Уточнить поиск`, toolbar/node cards use local sprite icons plus tokenized domain markers, and selecting a node now highlights its visible relationships, mutes unrelated canvas content and opens a compact workbench inspector with incoming/outgoing counts and `Открыть` / `Соседи` actions.
...
- Knowledge Graph is usable as a canvas workbench and now explains hidden canvas slices plus selected-node relationships, but file split, lifecycle cleanup and final overlay polish remain Phase 7/backlog work.
...
- Full browser smoke passed inside `npm run desktop:gate` with 110 browser tests after the AppShell rail/profile, Explorer action, block DnD, Add block popup, card editor toolbar, card-block select, template picker, command-palette/deep-search, campaign-map toolbar/popup/selection/layer/scene-inspector/group-action and Knowledge Graph visible-slice plus selected-node inspector corrections.
...
- `npm run test:browser -- tests/browser/knowledge-graph.spec.mjs` passed locally with 2 focused browser tests covering graph canvas operations, node/connect overlay lifecycle markers, visible-slice clarity, selected-node inspector content, active/muted edge states and context-menu anchor correction.
- `npm run test:browser -- tests/browser/campaign-map-ui.spec.mjs` passed locally with 15 browser tests, including the `0.0.1.8.12.1` toolbar grouping/accessibility/overflow guard, the `0.0.1.8.12.2` shared popup-frame guard, the `0.0.1.8.12.3` selection inspector/safe-action guard, the `0.0.1.8.12.4` layer/object dock guard, the `0.0.1.8.12.5` scene-state inspector/runtime-cleanup guard, the `0.0.1.8.12.6` group visibility action guard, plus existing map token, drawing, music, layers, fog and selection flows.
