...
- `npm run test:browser -- tests/browser/campaign-map-ui.spec.mjs` passed locally with 17 browser tests, including the `0.0.1.8.12.10` split toolbar/title/tool-rail guards for full-size zones, no internal scrollbars and unclipped floating tooltip labels, property Inspector and custom right-click guards, the `0.0.1.8.12.2` shared popup-frame guard, the `0.0.1.8.12.6` group visibility action guard, plus existing map token, drawing, music, layers, fog and selection flows.
