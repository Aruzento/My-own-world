...
- UI redesign Phase 8 secondary screens has continued through `0.0.1.8.14.3`: task tracker now uses a compact workbench toolbar, local sprite icon actions, quiet counters and guarded nested-icon button clicks; the Settings popup now presents appearance, backup, asset health and workspace diagnostics as one coherent maintenance panel with local sprite actions and explicit health/backup/restore/asset/danger markers; Tools help routes now open a Help/Support/Release guide with compact section navigation, support/release cards and an explicit planned import/export marker.
...
- `npm run verify` passed locally with 273 node tests and large-workspace smoke; the same verify step also passed inside `npm run desktop:gate`.
- Full browser smoke passed inside `npm run desktop:gate` with 114 browser tests after the AppShell rail/profile, Explorer action, block DnD, Add block popup, card editor toolbar, card-block select, template picker, command-palette/deep-search, campaign-map toolbar/popup/selection/group-action, Knowledge Graph `0.0.1.8.13.1`-`0.0.1.8.13.11` corrections and Phase 8 task tracker / Settings maintenance / Help-Support guards.
...
- `npm run test:browser -- tests/browser/app-shell.spec.mjs` and `tests/browser/visual-regression.spec.mjs --grep visual-safety-captures-core-surfaces` passed locally for the `0.0.1.8.14.3` Help/Support/Release guide, including compact Tools route markers, internal help navigation, planned import/export marker, no horizontal overflow and a visual screenshot guard.
...
- `npm run desktop:gate` passed locally, including docs index, skills validation, verify, 114 browser smoke tests, desktop frontend prepare, packaging smoke, desktop environment check and Tauri cargo check. The large-workspace desktop smoke was skipped because no external workspace path was provided.
