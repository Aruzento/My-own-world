...
- Full browser smoke passed inside `npm run desktop:gate` with 109 browser tests after the AppShell rail/profile, Explorer action, block DnD, Add block popup, card editor toolbar, card-block select, template picker, command-palette/deep-search and campaign-map toolbar/popup/selection/layer/scene-inspector/group-action corrections.
