...
- UI redesign Phase 7 Knowledge Graph has started with `0.0.1.8.13.1`: the graph canvas now shows visible/total/hidden counters, status says `показано X из Y`, hidden reasons separate filters, standard slice and canvas limit, the slice notice offers `Все связи` / `Уточнить поиск`, and toolbar/node cards use local sprite icons plus tokenized domain markers.
...
- Knowledge Graph is usable as a canvas workbench and now explains hidden canvas slices, but richer edge states, inspector work, file split and lifecycle cleanup remain Phase 7/backlog work.
...
- Full browser smoke passed inside `npm run desktop:gate` with 110 browser tests after the AppShell rail/profile, Explorer action, block DnD, Add block popup, card editor toolbar, card-block select, template picker, command-palette/deep-search, campaign-map toolbar/popup/selection/layer/scene-inspector/group-action and Knowledge Graph visible-slice corrections.
...
- `npm run test:browser -- tests/browser/knowledge-graph.spec.mjs` passed locally with 2 focused browser tests covering graph canvas operations, node/connect overlay lifecycle markers and visible-slice clarity.
