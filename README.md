...
Карта документации лежит в `docs/README.md`. Активные рабочие документы находятся в зонах `docs/00-product/`, `docs/01-delivery/`, `docs/02-architecture/`, `docs/03-testing/` и `docs/04-user-release/`. `docs/archive/` не является источником текущей правды: туда перенесены старые планы и эксперименты с причиной архивации.

Для задач с риском переусложнения использовать `.agents/skills/minimal-change/SKILL.md`: сначала проверить существующие модели, стандартную платформу и самый маленький безопасный патч.
...
- `editorOpenPage.js` открывает карточки, карты, таск-трекеры и Rule Tree;
- `editorSpecialSave.js` сохраняет спец-сущности: карту, таск-трекер и Rule Tree;
...
Rule Tree хранит правила мира как отдельную сущность `ruleTree` с JSON-моделью внутри `<script class="rule-tree-data" type="application/json" data-rule-tree-data>`. Создать ее можно вручную через главный `+` → `Правила` или через кнопку `Правила` на пустом стартовом экране. Legacy карточки с тегами `rule`, `rules`, `правило`, `правила` остаются источником импорта, но не являются целевой моделью правил. В карточках персонажей и существ блок `Состояния и эффекты` может хранить персональные `selectedRuleIds`, чтобы применять правила точечно через `CharacterModel`. Внутри Rule Tree можно редактировать группу, категорию, наследуемые rule id, package id и условия применения правила, а также экспортировать/импортировать JSON-пакет и смотреть предпросмотр активных эффектов. `RuleTreeEngine` проверяет условия `level`, `state`, `card-variable`, `manual`, `formula`, разворачивает наследование и передает в `CharacterModel` только применимые правила. Переносимые пакеты хранятся в workspace-папке `rule-packages/`; UI Rule Tree умеет сохранять текущий набор правил в package-файл, обновлять список файлов, импортировать файл, удалять файл и предупреждать о конфликте rule id. Панель диагностики показывает предупреждения наследования и краткое объяснение собственных/унаследованных условий и эффектов.
...
- `taskTracker`: отдельная доска задач;
- `ruleTree`: отдельное дерево правил и homebrew.

Все сущности хранятся как `.md`, но открываются разными renderer-ами.
...
- `creature` - КЗ, хиты, временные хиты, скорость, базовые характеристики, группы DnD-навыков, чувства и особенности;
...
- `character` - уровень, КЗ, хиты, временные хиты, скорость, базовые характеристики, хиты от смерти и группы DnD-навыков.
...
В блоке `Свойства` пользователь может открыть шестеренку, удалить лишнее видимое поле, добавить параметр из готовых названий расчетного слоя или создать свое поле. Поля внутри блока можно переставлять встроенной compact grip-ручкой, в том числе в пустую область сетки, и менять размер через маленькие точки по сторонам/углам. Во время переноса настоящее поле временно скрывается, под курсором идет runtime ghost, а сетка показывает placeholder будущей позиции; persistent DOM-порядок меняется только при drop. Layout поля сохраняется как model-first `data-property-layout` (`x/y/w/h/order/collapsed/groupId`), а `data-property-span` / `data-property-rows` остаются совместимыми производными для 12-колоночной сетки. Отдельной кнопки размера в шестеренке нет: размер меняется прямо на поле. Runtime grip/resize сделаны не кнопками, чтобы не мешать клику по input/textarea внутри поля. Runtime-кнопки, drag/resize handles, placeholder, ghost и popup-настройки не сохраняются как контент карточки.

Для `Персонаж` и `Существо` схема `Свойств` включает широкие мульти-поля `Навыки СИЛ`, `Навыки ЛОВ`, `Навыки ТЛС`, `Навыки ИНТ`, `Навыки МДР`, `Навыки ХАР`. Внутри них лежат строки спасбросков и навыков из старого `Стат. блок DnD`: числовое значение и checkbox владения. Эти строки сохраняются как стабильные переменные, например `skillAthletics`, `skillStealth`, `saveDex`, `skillAthleticsProficient`, поэтому будущие расчеты и Rule Tree смогут ссылаться на них без чтения русских подписей из HTML.

Навыки и спасброски в `Свойствах` пересчитываются автоматически: значение = модификатор характеристики + бонус мастерства, если включен checkbox владения. Ручной ввод в поле навыка или КЗ включает manual override и подсветку; если очистить поле, авторасчет вернется.

КЗ считается по DnD-правилам из ЛОВ и доспеха. У персонажа/существа есть поле `Доспех`, куда можно указать id/title/alias карточки-предмета. У предмета есть свойства доспеха: `Тип доспеха`, `Базовая КЗ доспеха`, `Лимит ЛОВ к КЗ`. Текущий UI использует текстовое поле ссылки на предмет; более удобный picker экипировки остается следующим этапом.

Новые stat-like данные больше не создаются через старые блоки `Статистика персонажа` и `Стат. блок DnD`: эти варианты скрыты из меню `Добавить блок`. Код старых блоков сохранен только как legacy-совместимость для уже существующих карточек.
...
`CharacterModel` foundation описан в `docs/02-architecture/contracts/CHARACTER_MODEL_CONTRACT.md` и реализован в `js/character/characterModel.js`. Сейчас он нормализует HP, временные HP, death saves, уровень, бонус мастерства, КЗ, скорость, базовые характеристики, инвентарь и активные эффекты/состояния из `PropertiesModel`, блока `Предметы` и `[data-character-effects]`, а legacy `Стат. блок DnD` остается fallback для старых карточек. Карта при чтении и изменении хитов по-прежнему идет через совместимый фасад `js/properties/characterCalculations.js`, чтобы старые сценарии не сломались. Связь карты с моделью лежит в `js/editor/campaignMapCharacterBridge.js`: через него карта получает HP, DEX-модификатор инициативы и эффекты из карточки.

Блок `Состояния и эффекты` хранит данные в безопасном JSON `[data-character-effects]`. Создать его можно вручную в карточке через `Добавить блок` → `Состояния и эффекты`; внутренний тип блока остается `characterEffects`. Через него можно добавить DnD-состояния, истощение, ручные эффекты и эффекты из карточек-источников: предметов, заклинаний и навыков. `CharacterModel` автоматически применяет эффекты предметов из инвентаря, если у карточки предмета есть собственный блок `Состояния и эффекты`. Связи с будущими Rule Tree и World Packages подготовлены через `sourceType`, `sourcePageId`, `sourcePackageId` и `ruleId`.

Блок `Лист персонажа` является runtime-экраном поверх `CharacterModel`: он показывает уровень, БМ, КЗ, скорость, инициативу, HP, death saves, характеристики, инвентарь и активные эффекты. Его поля можно редактировать, но отдельные игровые значения он не хранит: изменения записываются в блок `Свойства`, а лист затем заново строится из модели. Ручные override расчетных полей подсвечиваются и могут быть очищены прямо из листа, чтобы вернуть авторасчет.

Идея старых экспериментов `DnD v2` и `Переменные` теперь встроена в сущности карточек: блок `Свойства` задает переменные конкретной карточки, а `js/properties/cardVariablesModel.js` отдает их как единый model-first список. У разных типов карточек разные схемы свойств, но принцип один: стабильный ключ свойства является переменной сущности для расчетов, Rule Tree и будущих пакетов мира. Foundation `js/properties/cardVariableDependencies.js` уже умеет строить зависимости между карточками через пути вроде `self.str` и `race.str`, считать безопасные суммы без `eval` и возвращать diagnostics, если переменная или связанная карточка не найдена. Видимый UI конструктора формул остается будущим этапом.

Интеграции `CharacterModel` с будущими системами идут через `js/character/characterIntegrationApi.js`. Rule Tree и World Packages должны передавать эффекты как модельные данные, например через `createRuleTreeCharacterEffect()` или `createWorldPackageCharacterEffect()`, а не менять HTML карточки напрямую.

Первый Rule Tree provider находится в `js/rules/ruleTreeProvider.js`. На foundation-этапе rule-страницей считается обычная карточка с тегом `rule` / `rules` / `правило` / `правила`. Если на такой странице есть блок `Эффекты и состояния`, его эффекты можно подключить к персонажу через `selectedRuleIds`.

Целевое направление Rule Tree изменено: правила DnD должны стать внутренним `rules workspace` внутри корня программы, заполняемым с ролью `admin`. Пользователь будет открывать правила через wiki-link или через `Свойства` -> шестеренка -> popup дерева правил. Расчеты модификаторов, навыков, КЗ, хитов и эффектов остаются в коде приложения; человеко-читаемый аудит этих формул лежит в `docs/02-architecture/contracts/DND_CALCULATION_RULES.md`.

Internal rules workspace реализован в `js/rulesWorkspace/`. Правила поставляются как program-owned JSON `assets/rules/internal-rules-workspace.json`; `rulesWorkspaceSeed.js` остается fallback, если файл не загрузился. Правила не появляются в дереве мира и открываются как read-only `internalRule`.

Wiki-link сначала ищет обычную карточку мира, затем внутреннее правило по названию или alias. В шестеренке блока `Свойства` есть кнопка `Правила`: она раскрывает дерево внутреннего справочника, поддерживает поиск и открывает выбранное правило.

## Design System

Дизайн-система описана в `docs/02-architecture/ui/DESIGN_SYSTEM_CONTRACT.md`, а аудит и phased rollout лежат в `docs/02-architecture/ui/UI_AUDIT_AND_MODERNIZATION_PLAN.md`.
Продуктовый брендбук лежит в `docs/00-product/BRANDBOOK.md`.

Текущий визуальный слой называется `Archive Hearth`: теплая темная рабочая среда мастера НРИ без системного синего акцента. Базовые токены подключены через `styles/design-tokens.css`, а финальные общие состояния, hover/focus/active, popup motion и DnD-подсветки собраны в `styles/brand-system.css`.

Правило: новые блоки, вкладки, popup, панели и системы сначала смотрят брендбук и дизайн-контракт, затем используют существующие токены. Если нужен новый цвет или motion-паттерн, его нужно добавить в tokens/contract, а не хардкодить локально.
