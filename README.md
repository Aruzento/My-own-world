...
`CharacterModel` foundation описан в `docs/02-architecture/contracts/CHARACTER_MODEL_CONTRACT.md` и реализован в `js/character/characterModel.js`. Сейчас он нормализует HP, временные HP, death saves, уровень, бонус мастерства, КЗ, скорость, базовые характеристики и инвентарь из `PropertiesModel` / блока `Предметы`, а legacy `Стат. блок DnD` остается fallback для старых карточек. Карта при чтении и изменении хитов по-прежнему идет через совместимый фасад `js/properties/characterCalculations.js`, чтобы старые сценарии не сломались. Связь карты с моделью лежит в `js/editor/campaignMapCharacterBridge.js`: через него карта получает HP и DEX-модификатор инициативы из карточки.
