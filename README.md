...
Дерево запоминает свернутые родительские ветки в `localStorage`. При повторном открытии приложения состояние раскрытия восстанавливается, а id удалённых страниц автоматически вычищаются из сохранённого списка.
