...
Следующее развитие блока `Свойства` — `PropertiesModel`: отдельный слой чтения и расчетов, который будет получать данные из type-aware свойств, legacy `Стат. блок DnD` и будущих моделей персонажа без прямой привязки к HTML-разметке. Архивные эксперименты `Стат. блок DnD v2` и `Переменные` не возвращаются в интерфейс до появления такого model-first слоя.
