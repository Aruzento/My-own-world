...
Блок `Эффекты и состояния` хранит данные в безопасном JSON `[data-character-effects]`. Через него можно добавить DnD-состояния, истощение, ручные эффекты и эффекты из карточек-источников: предметов, заклинаний и навыков. `CharacterModel` автоматически применяет эффекты предметов из инвентаря, если у карточки предмета есть собственный блок `Эффекты и состояния`. Связи с будущими Rule Tree и World Packages подготовлены через `sourceType`, `sourcePageId`, `sourcePackageId` и `ruleId`.
