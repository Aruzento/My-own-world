...
test(
  ...
  async () => {
    ...
    for (const phrase of [
      ...
      'js/events/eventQuery.js',
      'queryEventLog(query, { storageAdapter, strict })',
      'getEventTransactionById(transactionId, { storageAdapter, strict })',
      'recent events by default',
      'cursor pagination',
      'filter by `entityId`',
      'durable `logOrder` range',
      'consumers must not parse `.my-own-world-events/transactions.v1.jsonl` directly',
      ...
      'the durable store must not import future adapters',
      'action.*',
      'healing.*',
      'effect.*',
      'turn.*',
      ...
      'rest.*',
      'movement.*',
      'tests/eventFutureAdapterContract.test.mjs',
      'reservedFuture',
      'does not implement action, damage, healing, effect, turn, round, rest, movement, scene-transition behavior',
      '.my-own-world-events/',
