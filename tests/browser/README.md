...
- `visual-regression.spec.mjs` now also guards the `0.0.1.8.11.7` core-content contract and the `0.0.1.8.12.1` / `0.0.1.8.12.2` campaign-map baselines: card editor header/toolbar local icons and overlay placement, shared card block type badges/markers, shared card-block select styling, command palette/deep search styling, map toolbar grouping/fit through visual map capture, map popup shared-frame coverage, plus the updated Properties visual baseline.
