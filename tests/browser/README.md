...
- `component-catalogue.spec.mjs` verifies the dev/test component catalogue route, shared Button/IconButton/Field/Toolbar/Panel/Popover states, density response, focus, Escape close, the app Tools shared Button consumer and overlay state markers. Normal Tools usage must not expose the catalogue unless `my-own-world:show-component-catalogue` is enabled.
...
- `visual-regression.spec.mjs` now also guards the `0.0.1.8.17` owner visual completion matrix as evidence smoke, not strict pixel comparison: card editor header/toolbar local icons and overlay placement, shared card block type badges/markers, shared card-block select styling, command palette/deep search styling, split full-size map scene bar/tool rail and title-chip fit through visual map capture, map toolbar floating tooltip/no-scrollbar coverage, map popup shared-frame coverage, right-side property Inspector capture, popup/Inspector non-overlap, custom right-click menu coverage, no duplicate scene/layer stage panels, the graph node context menu screenshot, quieter task tracker empty-board states, Settings/diagnostics screenshots and updated Properties visual baseline.
...
- `visual-regression.spec.mjs` — сохраняет screenshot attachments ключевых поверхностей из `UI_MIGRATION_BASELINES.md`, включая `visual-add-block-popup`, и проверяет базовые visual guards: popup в viewport, фиксированную ширину toolbar, selection-box карты, слой тумана и badge `скрыт`. Это evidence smoke, а не strict pixel baseline.
